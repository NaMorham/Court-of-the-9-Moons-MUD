...
static int pop(struct stack_data *stack)
{
    if (S_LEN(stack) > 0) {
        ...
        WriteLogf("SYSERR: Illegal expression %d in shop keyword list.", S_LEN(stack));
        return (0);
    }
}
...
static int evaluate_expression(struct obj_data *obj, char *expr)
{
            ...
            if ((temp = find_oper_num(*ptr)) == NOTHING) {
                ...
                if (temp == OPER_CLOSE_PAREN) {
                    if ((temp = pop(&ops)) != OPER_OPEN_PAREN) {
                        WriteLogf("SYSERR: Illegal parenthesis in shop keyword expression.");
                        return (FALSE);
                    }
                }
                else {
    ...
    temp = pop(&vals);
    if (top(&vals) != -1) {
        WriteLogf("SYSERR: Extra operands left on shop keyword expression stack.");
        return (FALSE);
    }
    return (temp);
}
...
static int trade_with(struct obj_data *item, int shop_nr)
{
    ...
    else if (OBJ_FLAGGED(item, ITEM_NOSELL)) {
        return (OBJECT_NOTOK);
    }
...
static int same_obj(struct obj_data *obj1, struct obj_data *obj2)
{
    ...
    else if (GET_OBJ_RNUM(obj1) != GET_OBJ_RNUM(obj2)) {
        return (FALSE);
    }
...
static struct obj_data *get_selling_obj(struct char_data *ch, char *name, struct char_data *keeper, int shop_nr, int msg)
{
    ...
    case OBJECT_DEAD:
        ...
        break;
    default:
        WriteLogf("SYSERR: Illegal return value of %d from trade_with() (%s)", result, __FILE__);    /* Someone might rename it... */
...
{
    if (error) {
        WriteLogf("SYSERR: Raise MAX_SHOP_OBJ constant in shop.h to %d", len + error);
    }
    BUY_WORD(list[len]) = NULL;
...
static void read_line(FILE *shop_f, const char *string, void *data)
{
    ...
    if (!get_line(shop_f, buf) || !sscanf(buf, string, data)) {
        WriteLogf("SYSERR: Error in shop #%d, near '%s' with '%s'", SHOP_NUM(top_shop), buf, string);
        exit(1);
    }
}
...
static char *read_shop_message(int mnum, room_vnum shr, FILE *shop_f, const char *why)
{
    ...
    for (cht = 0; tbuf[cht]; cht++) {
        ...
        if (tbuf[cht + 1] == 's') {
            ...
            if (ss == 0) {
                WriteLogf("SYSERR: Shop #%d has %%d before %%s, message #%d.", shr, mnum);
                err++;
            }
            ds++;
        }
        else if (tbuf[cht + 1] != '%') {
            WriteLogf("SYSERR: Shop #%d has invalid format '%%%c' in message #%d.", shr, tbuf[cht + 1], mnum);
            err++;
        }
    }  // for (cht ...

    if ((ss > 1) || (ds > 1)) {
        WriteLogf("SYSERR: Shop #%d has too many specifiers for message #%d. %%s=%d %%d=%d", shr, mnum, ss, ds);
        err++;
    }
...
void assign_the_shopkeepers(void)
{
    ...
    for (cindex = 0; cindex <= top_shop; cindex++) {
        ...
        if (SHOP_KEEPER(cindex) > top_of_mobt) {
            WriteLogf("shop %d had mob out of bounds", cindex);
            abort();
        }
        ...
        if (shop_index[cnt].type) {
            for (itr = 0; BUY_TYPE(shop_index[cnt].type[itr]) != NOTHING; itr++) {
                if (BUY_WORD(shop_index[cnt].type[itr])) {
                    ...
                    BUY_WORD(shop_index[cnt].type[itr]) = NULL;
                }
            }  // for (itr ...
            free(shop_index[cnt].type);
            shop_index[cnt].type = NULL;
        }
    }  // for (cnt ...
