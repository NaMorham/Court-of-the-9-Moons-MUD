...
room_rnum add_room(struct room_data *room)
{
    ...
    if ((i = real_room(room->number)) != NOWHERE) {
        ...
        WriteLogf("GenOLC: add_room: Updated existing room #%d.", room->number);
        return i;
    }
    ...
    WriteLogf("GenOLC: add_room: Added room %d at index #%d.", room->number, found);
...
int delete_room(room_rnum rnum)
{
    ...
    WriteLogf("GenOLC: delete_room: Deleting room #%d (%s).", room->number, room->name);

    if (r_mortal_start_room == rnum) {
        WriteLogf("WARNING: GenOLC: delete_room: Deleting mortal start room!");
        r_mortal_start_room = 0;    // The Void
    }
    if (r_immort_start_room == rnum) {
        WriteLogf("WARNING: GenOLC: delete_room: Deleting immortal start room!");
        r_immort_start_room = 0;    // The Void
    }
    if (r_frozen_start_room == rnum) {
        WriteLogf("WARNING: GenOLC: delete_room: Deleting frozen start room!");
    ...
    do {
        ...
        for (j = 0; j < NUM_OF_DIRS; j++) {
            ...
            else if (exit_data->to_room > rnum) {
                exit_data->to_room -= (exit_data->to_room != NOWHERE);  // with unsigned NOWHERE > any rnum
            }
            else if (exit_data->to_room == rnum) {
                if ((!exit_data->keyword || !*exit_data->keyword) &&
                    (!exit_data->general_description || !*exit_data->general_description)) {
...
#endif
        WriteLogf("SYSERR: GenOLC: save_rooms: Invalid zone number %d passed! (0-%d)", rzone, top_of_zone_table);
        return FALSE;
    }

    WriteLogf("GenOLC: save_rooms: Saving rooms in zone #%d (%d-%d).",
...
int copy_room_strings(struct room_data *dest, struct room_data *source)
{
    ...
    if (dest == NULL || source == NULL) {
        WriteLogf("SYSERR: GenOLC: copy_room_strings: NULL values passed.");
        return FALSE;
    }
