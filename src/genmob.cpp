...
int add_mobile(struct char_data *mob, mob_vnum vnum)
{
    ...
    if ((rnum = real_mobile(vnum)) != NOBODY) {
        ...
        WriteLogf("GenOLC: add_mobile: Updated existing mobile #%d.", vnum);
        return rnum;
    }
    ...
    WriteLogf("GenOLC: add_mobile: Added mobile %d at index #%d.", vnum, found);
...
#endif
        WriteLogf("SYSERR: GenOLC: delete_mobile: Invalid rnum %d.", refpt);
        return NOBODY;
    }
...
int save_mobiles(zone_rnum rznum)
{
...
#endif
        WriteLogf("SYSERR: GenOLC: save_mobiles: Invalid real zone number %d. (0-%d)", rznum, top_of_zone_table);
        return FALSE;
    }
    ...
    for (i = genolc_zone_bottom(rznum); i <= zone_table[rznum].top; i++) {
        ...
        if (write_mobile_record(i, &mob_proto[rmob], mobfd) < 0) {
            WriteLogf("SYSERR: GenOLC: Error writing mobile #%d.", i);
        }
    }  // for (i ...
    ...
    WriteLogf("GenOLC: '%s' saved, %d bytes written.", usedfname, written);
    return written;
}
