...
int copy_quest_strings(struct aq_data *to, struct aq_data *from)
{
    if ((from == NULL) || (to == NULL)) {
        WriteLogf("SYSERR: GenQST: copy_quest_strings: Null values passed.");
        return FALSE;
    }
...
int delete_quest(qst_rnum rnum)
{
    ...
    WriteLogf("GenOLC: delete_quest: Deleting quest #%d (%s).", QST_NUM(rnum), QST_NAME(rnum));
...
#endif
        WriteLogf("SYSERR: GenOLC: save_quests: Invalid zone number %d passed! (0-%d)",
            zone_num, top_of_zone_table);
        return FALSE;
    }

    WriteLogf("GenOLC: save_quests: Saving quests in zone #%d (%d-%d).",
