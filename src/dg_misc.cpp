    ...
    int script_type, char *cmd)
{
    ...
    char charname[MAX_INPUT_LENGTH], propertyname[MAX_INPUT_LENGTH];
    ...
    half_chop(cmd, propertyname, cmd);
    ...
    if (duration <= 0) {
        ...
        script_log("Line was: dg_affect %s %s %s %s (%d)",
            charname, propertyname, value_p, duration_p, duration);
        return;
    }
    ...
    while (str_cmp(apply_types[i], "\n")) {
        if (!str_cmp(apply_types[i], propertyname)) {
            type = APPLY_TYPE;
            break;
        }
        i++;
    ...
    if (!type) {  // search affect_types now
        ...
        while (str_cmp(affected_bits[i], "\n")) {
            if (!str_cmp(affected_bits[i], propertyname)) {
                type = AFFECT_TYPE;
                break;
            }
            i++;
    ...
    if (!type) {  // property not found
        script_log("Trigger: %s, VNum %d. dg_affect: unknown property '%s'!",
            GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), propertyname);
        return;
    }
...
int valid_dg_target(struct char_data *ch, int bitvector)
{
    ...
    else if (!IS_SET(bitvector, DG_ALLOW_GODS) && GET_LEVEL(ch) >= LVL_GRGOD) {  // LVL_GOD has the advance command. Can't allow them to be forced.
