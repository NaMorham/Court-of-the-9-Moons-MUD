    ...
    if ((txt = findLine(plr_file, "Flag:")) != NULL) {
        ...
        if (sscanf(txt, "%s %s %s %s", f1, f2, f3, f4) == 4) {
            ...
            if (IS_SET_AR(fl, MAIL_DELETED)) {
                SET_BIT(ret, MINDEX_DELETED);
            }
            if (IS_SET_AR(fl, MAIL_URGENT)) {
                SET_BIT(ret, MINDEX_URGENT);
            }
            if (IS_SET_AR(fl, MAIL_COD)) {
                SET_BIT(ret, MINDEX_IS_COD);
            }
            if (IS_SET_AR(fl, MAIL_FROM_MOB)) {
                SET_BIT(ret, MINDEX_FROM_MOB);
            }
            if (IS_SET_AR(fl, MAIL_READ)) {
                SET_BIT(ret, MINDEX_READ);
            }
            if (IS_SET_AR(fl, MAIL_DRAFT)) {
                SET_BIT(ret, MINDEX_DRAFT);
            }
        }
    }
