...
// local functions
static void remove_cmd_from_list(ResetCommand_t **list, int pos);
...
void create_world_index(int znum, const char *type)
{
    ...
    default:
        // Caller messed up.
        WriteLogf("SYSERR: Unknown type %s", type);
        return;
    }
...
void remove_room_zone_commands(zone_rnum zone, room_rnum room_num)
{
    ...
    while (zone_table[zone].cmd[subcmd].command != 'S') {
        ...
        case 'R':
            ...
            break;
        default:
            WriteLogf("SYSERR: Unknown command type [%c] in cone [%d]",
                zone_table[zone].cmd[subcmd].command, zone_table[zone].number);
            break;
        }
...
#endif
        WriteLogf("SYSERR: GenOLC: save_zone: Invalid real zone number %d. (0-%d)", zone_num, top_of_zone_table);
        return FALSE;
    }
 ...
 */
int count_commands(ResetCommand_t *list)
{
    int count = 0;
 ...
 */
void add_cmd_to_list(ResetCommand_t **list, ResetCommand_t *newcmd, int pos)
{
    int count, i, l;
    ResetCommand_t *newlist;
    ...
    CREATE(newlist, ResetCommand_t, count + 2);
...
static void remove_cmd_from_list(ResetCommand_t **list, int pos)
{
    ...
    CREATE(newlist, ResetCommand_t, count);
