...
void load_messages(void)
{
    ...
    while (!feof(fl)) {
        ...
        while (*chk == 'M') {
            buf = fgets(chk, 128, fl);
            sscanf(chk, " %d\n", &type);
            for (i = 0; (i < MAX_MESSAGES) && (fight_messages[i].a_type != type) && (fight_messages[i].a_type); i++) { }  // find the right message
            if (i >= MAX_MESSAGES) {
...
int damage(struct char_data *ch, struct char_data *victim, int dam, int attacktype)
{
    ...
    if (GET_POS(victim) <= POS_DEAD) {
        ...
        WriteLogf("SYSERR: Attempt to damage corpse '%s' in room #%d by '%s'.",
