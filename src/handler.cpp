...
void aff_apply_modify(struct char_data *ch, byte loc, sbyte mod, char *msg)
{
    ...
    case APPLY_STR:
        ch->aff_abils.setStr(GET_STR(ch) + mod);
        break;

    case APPLY_DEX:
        ch->aff_abils.setDex(GET_DEX(ch) + mod);
        break;

    case APPLY_INT:
        ch->aff_abils.setIntel(GET_INT(ch) + mod);
        break;

    case APPLY_WIS:
        ch->aff_abils.setWis(GET_WIS(ch) + mod);
        break;

    case APPLY_CON:
        ch->aff_abils.setCon(GET_CON(ch) + mod);
        break;
    ...
    default:
        WriteLogf("SYSERR: Unknown apply adjust %d attempt (%s, affect_modify).", loc, __FILE__);
        break;
    }  // switch
}
...
void affect_total(struct char_data *ch)
{
    ...
    ch->aff_abils.setDex(MAX(0, MIN(GET_DEX(ch), i)));
    ch->aff_abils.setIntel(MAX(0, MIN(GET_INT(ch), i)));
    ch->aff_abils.setWis(MAX(0, MIN(GET_WIS(ch), i)));
    ch->aff_abils.setCon(MAX(0, MIN(GET_CON(ch), i)));
    ...
    if (IS_NPC(ch)) {
        ...
        if (GET_STR(ch) > 18) {
            ...
            ch->aff_abils.setStrAdd(MIN(i, 100));
            ch->aff_abils.setStr(18);
        }
    }
}
...
void obj_from_char(struct obj_data *object)
{
    ...
    if (object == NULL) {
        WriteLogf("SYSERR: NULL object passed to obj_from_char.");
        return;
    }
...
int invalid_align(struct char_data *ch, struct obj_data *obj)
{
    ...
    else if (OBJ_FLAGGED(obj, ITEM_ANTI_GOOD) && IS_GOOD(ch)) {
        return TRUE;
    }
    else if (OBJ_FLAGGED(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(ch)) {
        return TRUE;
    }
    return FALSE;
}
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
    ...
    if (GET_EQ(ch, pos)) {
        WriteLogf("SYSERR: Char is already equipped: %s, %s", GET_NAME(ch),
            obj->short_description);
        return;
    }
    if (obj->carried_by) {
        WriteLogf("SYSERR: EQUIP: Obj is carried_by when equip.");
        return;
    }
    if (IN_ROOM(obj) != NOWHERE) {
        WriteLogf("SYSERR: EQUIP: Obj is in_room when equip.");
        return;
    }
    ...
    if (IN_ROOM(ch) != NOWHERE) {
        ...
        WriteLogf("SYSERR: IN_ROOM(ch) = NOWHERE when equipping char %s.", GET_NAME(ch));
    }
...
struct obj_data *unequip_char(struct char_data *ch, int pos)
{
    ...
    if (IN_ROOM(ch) != NOWHERE) {
        ...
        WriteLogf("SYSERR: IN_ROOM(ch) = NOWHERE when unequipping char %s.", GET_NAME(ch));
    }
...
void obj_from_obj(struct obj_data *obj)
{
    ...
    if (obj->in_obj == NULL) {
        WriteLogf("SYSERR: (%s): trying to illegally extract obj from obj.", __FILE__);
        return;
    }
    obj_from = obj->in_obj;
    temp = obj->in_obj;
...
void extract_obj(struct obj_data *obj)
{
    ...
    if (obj->worn_by != NULL) {
        if (unequip_char(obj->worn_by, obj->worn_on) != obj) {
            WriteLogf("SYSERR: Inconsistent worn_by and worn_on pointers!!");
        }
    }
    if (IN_ROOM(obj) != NOWHERE) {
        obj_from_room(obj);
...
void extract_char_final(struct char_data *ch)
{
    ...
    if (IN_ROOM(ch) == NOWHERE) {
        WriteLogf("SYSERR: NOWHERE extracting char %s. (%s, extract_char_final)",
...
void extract_pending_chars(void)
{
    ...
    if (extractions_pending < 0) {
        WriteLogf("SYSERR: Negative (%d) extractions pending.", extractions_pending);
    }
...
struct char_data *get_char_world_vis(struct char_data *ch, char *name, int *number)
{
    ...
    for (i = character_list; i && *number; i = i->next) {
        ...
        else if (!isname(name, i->player.name)) {
            continue;
        }
...
const char *money_desc(int amount)
{
    ...
    if (amount <= 0) {
        WriteLogf("SYSERR: Try to create negative or 0 money (%d).", amount);
        return (NULL);
    }
