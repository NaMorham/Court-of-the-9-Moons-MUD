...
ACMD(do_shutdown)
{
    ...
    if (!*arg) {
        WriteLogf("(GC) Shutdown by %s.", GET_NAME(ch));
        send_to_all("Shutting down.\r\n");
        circle_shutdown = 1;
    }
    else if (!str_cmp(arg, "reboot")) {
        WriteLogf("(GC) Reboot by %s.", GET_NAME(ch));
        ...
        WriteLogf("(GC) Shutdown NOW by %s.", GET_NAME(ch));
...
ACMD(do_load)
{
    ...
    if (is_abbrev(buf, "mob")) {
        ...
        // normal imms are only allowed to load mobs in areas they can edit
        ...
        // normal imms are only allowed to load objs in areas they can edit
        if (GET_LEVEL(ch) < LVL_GRGOD && !can_edit_zone(ch, world[IN_ROOM(ch)].zone)) {
...
ACMD(do_purge)
{
    ...
    // normal imms are only allowed to purge in areas they can edit
    if (GET_LEVEL(ch) < LVL_GRGOD && !can_edit_zone(ch, world[IN_ROOM(ch)].zone)) {
...
ACMD(do_advance)
{
    ...
    if (newlevel < oldlevel) {
        WriteLogf("(GC) %s demoted %s from level %d to %d.",
            GET_NAME(ch), GET_NAME(victim), oldlevel, newlevel);
    }
    else {
        WriteLogf("(GC) %s has advanced %s to level %d (from %d)",
    ...
    else if (is_abbrev(buf, "all"))
    {
        ...
        if (!IS_NPC(vict) && GET_LEVEL(ch) >= LVL_GRGOD) {
            ...
            if (GET_LEVEL(vict) >= LVL_GRGOD) {
                vict->real_abils.setStr(25);
                vict->real_abils.setStrAdd(100);
                vict->real_abils.setIntel(25);
                vict->real_abils.setWis(25);
                vict->real_abils.setDex(25);
                vict->real_abils.setCon(25);
                vict->real_abils.setCha(25);
            }
        }
        update_pos(vict);
        affect_total(vict);
...
ACMD(do_dc)
{
    ...
    if (STATE(d) == CON_DISCONNECT || STATE(d) == CON_CLOSE) {
        ...
        WriteLogf("(GC) Connection closed by %s.", GET_NAME(ch));
    }
}
...
static void mod_llog_entry(struct last_entry *llast, int type) {
    ...
    for (tmp = recs; tmp > 0; tmp--) {
        fseek(fp, -1 * ((long)sizeof(struct last_entry)), SEEK_CUR);
        if (fread(&mlast, sizeof(struct last_entry), 1, fp) != 1) {
            WriteLogf("mod_llog_entry: read error or unexpected end of file.");
            return;
        }
        // Another one to keep that stepback.
        fseek(fp, -1 * long(sizeof(struct last_entry)), SEEK_CUR);
...
void add_llog_entry(struct char_data *ch, int type) {
    ...
    if (llast == NULL) {  // no entry found, add ..error if close!
        ...
        if (!(fp = fopen(LAST_FILE, "a"))) {
            WriteLogf("error opening last_file for appending");
            free(llast);
            llast = NULL;
            return;
        }
...
void clean_llog_entries(void) {
    ...
    if (!(nfp = fopen("etc/nlast", "w"))) {
        WriteLogf("Error trying to open new last file.");
        fclose(ofp);
        return;
    }
...
static void list_llog_entries(struct char_data *ch)
{
    ...
    if (!(fp = fopen(LAST_FILE, "r"))) {
        WriteLogf("llist_log_entries: could not open last log file %s.", LAST_FILE);
...
ACMD(do_wizutil)
{
    ...
    if (!*arg) {
        ...
        case SCMD_REROLL:
            ...
            WriteLogf("(GC) %s has rerolled %s.", GET_NAME(ch), GET_NAME(vict));
        ...
        case SCMD_UNAFFECT:
            ...
            break;
        default:
            WriteLogf("SYSERR: Unknown subcmd %d passed to do_wizutil (%s)", subcmd, __FILE__);
            // SYSERR_DESC: This is the same as the unhandled case in do_gen_ps(), but this function handles 'reroll', 'pardon', 'freeze', etc.
            break;
        }
        save_char(vict);
    }
...
/*
 * single zone printing fn used by "show zone" so it's not repeated in the code 3 times ... -je, 4/6/93 FIXME: overflow possible
 */
static size_t print_zone_to_buf(char *bufptr, size_t left, zone_rnum zone, int listall)
{
    size_t tmp;
...
ACMD(do_show)
{
    ...
    case 1:
        ...
        if (self) {
            ...
            for (zvn = atoi(value), zrn = 0; zone_table[zrn].number != zvn && zrn <= top_of_zone_table; zrn++) { } // search through zone table
...
static int perform_set(struct char_data *ch, struct char_data *vict, int mode, char *val_arg)
{
    ...
    case 5: // brief
        ...
        break;
    case 6:  // cha
        // @todo: Adjust range limits based on race/class
    ...
    case 21: // int
        ...
        vict->real_abils.setIntel(value);
        affect_total(vict);
        break;
    case 22: // invis
    ...
    case 48: // str
        ...
        vict->real_abils.setStr(value);
        vict->real_abils.setStrAdd(0);
        affect_total(vict);
        break;
    case 49: // stradd
        // @todo: Adjust range limits based on race/class
        vict->real_abils.setStrAdd(RANGE(0, 100));
        if (value > 0) {
            vict->real_abils.setStr(18);
        }
        affect_total(vict);
        break;
    case 50: // thief
...
ACMD(do_zcheck)
{
    ...
    for (i = 0; i < top_of_mobt; i++) {
        if (real_zone_by_thing(mob_index[i].vnum) == zrnum) {  // is mob in this zone?
            ...
            if (MOB_FLAGGED(mob, MOB_AGGRESSIVE) && (MOB_FLAGGED(mob, MOB_AGGR_GOOD) ||
                MOB_FLAGGED(mob, MOB_AGGR_EVIL) || MOB_FLAGGED(mob, MOB_AGGR_NEUTRAL)) && (found = 1)) {
                len += snprintf(buf + len, sizeof(buf) - len,
    ...
    for (i = 0; i < top_of_world; i++) {
        if (world[i].zone == zrnum) {
            ...
            if (ROOM_FLAGGED(i, ROOM_ATRIUM) || ROOM_FLAGGED(i, ROOM_HOUSE) ||
                ROOM_FLAGGED(i, ROOM_HOUSE_CRASH) || ROOM_FLAGGED(i, ROOM_OLC) || ROOM_FLAGGED(i, ROOM_BFS_MARK)) {
                len += snprintf(buf + len, sizeof(buf) - len,
...
ACMD(do_copyover)
{
    ...
    basic_mud_log("Attempt to do_copyover on windows");
#else
    // TOO DANGEROUS - TOO MANY ASSUMPTIONS
    FILE *fp;
...
ACMD(do_file)
{
    ...
    if (!*value) {
        ...
        // This check forces the requisite positive digit and prevents negative numbers of lines from being read.
    ...
    // Since file_head and file_tail will add the overflow message, we don't check for status here.
...
bool change_player_name(struct char_data *ch, struct char_data *vict, char *new_name)
{
    ...
    for (k = 0; (*(player_table[i].name + k) = LOWER(*(player_table[i].name + k))); k++) { } // search pleyer table
...
static int get_max_recent(void)
{
    struct recent_player *this_plr;
    int iRet=0;

    this_plr = recent_list;

    while (this_plr)
    {
        if (this_plr->vnum > iRet) {
            iRet = this_plr->vnum;
        }
        this_plr = this_plr->next;
    }
 ...
 */
static void clear_recent(struct recent_player *this_plr)
{
    this_plr->vnum = 0;
    this_plr->time = 0;
    strcpy(this_plr->name, "");
    strcpy(this_plr->host, "");
    this_plr->next = NULL;
}
...
bool AddRecentPlayer(char *chname, char *chhost, bool newplr, bool cpyplr)
{
    ...
    this_plr->time = ct;
    this_plr->new_player = newplr;
    this_plr->copyover_player = cpyplr;
    strcpy(this_plr->host, chhost);
    strcpy(this_plr->name, chname);
    max_vnum = get_max_recent();
    this_plr->vnum = max_vnum;    // Possibly should be +1 ?
...
ACMD(do_recent)
{
    ...
    while (this_plr)
    {
        ...
        if (*(this_plr->host)) {
            if (!strcmp(this_plr->host, "localhost")) {
                loc = TRUE;
            }
        }
        ...
        if ((limit == 0) || (count < limit)) {
            if (GET_LEVEL(ch) >= LVL_GRGOD) {  // If High-Level Imm, then show Host IP
                if (this_plr->new_player == TRUE) {
                    send_to_char(ch, "%3d | %-24.24s | %s%-32s%s | %s %s(New Player)%s\r\n", this_plr->vnum, timestr, loc ? QRED : "", this_plr->host, QNRM, this_plr->name, QYEL, QNRM);
                }
                else if (this_plr->copyover_player == TRUE) {
                    send_to_char(ch, "%3d | %-24.24s | %s%-32s%s | %s %s(Copyover)%s\r\n", this_plr->vnum, timestr, loc ? QRED : "", this_plr->host, QNRM, this_plr->name, QCYN, QNRM);
                }
                else {
                    send_to_char(ch, "%3d | %-24.24s | %s%-32s%s | %s\r\n", this_plr->vnum, timestr, loc ? QRED : "", this_plr->host, QNRM, this_plr->name);
                }
            }
            else {
                if (this_plr->new_player == TRUE) {
                    send_to_char(ch, "%3d | %-24.24s | %s %s(New Player)%s\r\n", this_plr->vnum, timestr, this_plr->name, QYEL, QNRM);
                }
                else if (this_plr->copyover_player == TRUE) {
                    send_to_char(ch, "%3d | %-24.24s | %s %s(Copyover)%s\r\n", this_plr->vnum, timestr, this_plr->name, QCYN, QNRM);
                }
                else {
                    send_to_char(ch, "%3d | %-24.24s | %s\r\n", this_plr->vnum, timestr, this_plr->name);
                }
            }
            count++;
