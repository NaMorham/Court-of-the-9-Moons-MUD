...
void command_interpreter(struct char_data *ch, char *argument)
{
    ...
    // here has been removed. Otherwise, find the command.
    {
        ...
        if (!cont) {    // any mobile triggers ?
            cont = command_mtrigger(ch, arg, line);
        }
        if (!cont) {    // any object triggers ?
...
char *one_argument(char *argument, char *first_arg)
{
    ...
    if (!argument) {
        WriteLogf("SYSERR: one_argument received a NULL pointer!");
        *first_arg = '\0';
        return (NULL);
    }
...
void nanny(struct descriptor_data *d, char *arg)
{
        ...
        if (!*arg) {
            ...
            if ((player_i = load_char(tmp_name, d->character)) > -1) {
                ...
                if (PLR_FLAGGED(d->character, PLR_DELETED)) {
                    // Make sure old files are removed so the new player doesn't get the deleted player's equipment.
    ...
    default:
        WriteLogf("SYSERR: Nanny: illegal state of con'ness (%d) for '%s'; closing connection.",
