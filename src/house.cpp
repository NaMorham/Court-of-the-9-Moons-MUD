...
static int House_load(room_vnum vnum)
{
    ...
    else if (!House_get_filename(vnum, filename, sizeof(filename))) {
        return (0);
    }
    else if (!(fl = fopen(filename, "r"))) {  // no file found
        return (0);
    }
...
void House_crashsave(room_vnum vnum)
{
    ...
    else if (!House_get_filename(vnum, buf, sizeof(buf))) {
        return;
    }
    else if (!(fp = fopen(buf, "wb"))) {
        perror("SYSERR: Error saving house file");
        return;
    }
    else if (!House_save(world[rnum].contents, fp)) {
        fclose(fp);
        return;
    }
    fclose(fp);
...
static void House_delete_file(room_vnum vnum)
{
    ...
    if (!(fl = fopen(filename, "rb"))) {
        if (errno != ENOENT) {
            WriteLogf("SYSERR: Error deleting house file #%d. (1): %s", vnum, strerror(errno));
        }
        return;
    }
    fclose(fl);
    if (remove(filename) < 0) {
        WriteLogf("SYSERR: Error deleting house file #%d. (2): %s", vnum, strerror(errno));
    }
}
...
void House_boot(void)
{
    ...
    if (!(fl = fopen(HCONTROL_FILE, "rb"))) {
        if (errno == ENOENT) {
            WriteLogf("   No houses to load. File '%s' does not exist.", HCONTROL_FILE);
        }
        else {
    ...
    while (!feof(fl) && (num_of_houses < MAX_HOUSES)) {
        ...
        else if (get_name_by_id(temp_house.owner) == NULL) {
            continue;            // owner no longer exists -- skip
        }
        else if ((real_house = real_room(temp_house.vnum)) == NOWHERE) {
            continue;            // this vnum doesn't exist -- skip
        }
        else if (find_house(temp_house.vnum) != NOWHERE) {
            continue;            // this vnum is already a house -- skip
        }
        else if ((real_atrium = real_room(temp_house.atrium)) == NOWHERE) {
            continue;            // house doesn't have an atrium -- skip
        }
        else if ((temp_house.exit_num < 0) || (temp_house.exit_num >= NUM_OF_DIRS)) {
            continue;            // invalid exit num -- skip
        }
        else if (TOROOM(real_house, temp_house.exit_num) != real_atrium) {
...
static void hcontrol_destroy_house(struct char_data *ch, char *arg)
{
    ...
    if ((real_atrium = real_room(house_control[i].atrium)) == NOWHERE) {
        WriteLogf("SYSERR: House %d had invalid atrium %d!", atoi(arg), house_control[i].atrium);
    }
    else {
    ...
    if ((real_house = real_room(house_control[i].vnum)) == NOWHERE) {
        WriteLogf("SYSERR: House %d had invalid vnum %d!", atoi(arg), house_control[i].vnum);
    }
    else {
