...
int save_all(void)
{
    while (save_list) {
        if (save_list->type < 0 || save_list->type > SL_MAX) {
            ...
            case SL_ACT:
                WriteLogf("Actions not saved - can not autosave. Use 'aedit save'.");
                save_list = save_list->next;    // Fatal error, skip this one.
                break;
            case SL_HLP:
                WriteLogf("Help not saved - can not autosave. Use 'hedit save'.");
                save_list = save_list->next;    // Fatal error, skip this one.
                break;
            default:
                WriteLogf("SYSERR: GenOLC: Invalid save type %d in save list.\n", save_list->type);
                break;
            }
        }
...
void free_ex_descriptions(struct extra_descr_data *head)
{
    ...
    if (!head) {
        WriteLogf("free_ex_descriptions: NULL pointer or NULL data.");
        return;
    }
...
int remove_from_save_list(zone_vnum zone, int type)
{
    ...
    if (ritem == NULL) {
        WriteLogf("SYSERR: remove_from_save_list: Saved item not found. (%d/%d)", zone, type);
        return FALSE;
    }
...
int add_to_save_list(zone_vnum zone, int type)
{
    ...
    if (rznum == NOWHERE || rznum > top_of_zone_table) {
        if (zone != AEDIT_PERMISSION && zone != HEDIT_PERMISSION) {
            WriteLogf("SYSERR: add_to_save_list: Invalid zone number passed. (%d => %d, 0-%d)", zone, rznum, top_of_zone_table);
            return FALSE;
        }
    }
...
static int export_save_mobiles(zone_rnum rznum)
{
    ...
    for (i = genolc_zone_bottom(rznum); i <= zone_table[rznum].top; i++) {
        ...
        if (export_mobile_record(i, &mob_proto[rmob], mob_file) < 0) {
            WriteLogf("SYSERR: export_save_mobiles: Error writing mobile #%d.", i);
        }
    }  // for (i ...
    fputs("$\n", mob_file);
    fclose(mob_file);
...
static int export_save_rooms(zone_rnum zrnum)
{
    ...
    }  // for (i = genolc_zone_bottom(zrnum) ...
...
static void export_script_save_to_disk(FILE *fp, void *item, int type)
{
    ...
    if (type == MOB_TRIGGER) {
        ...
        WriteLogf("SYSERR: Invalid type passed to export_script_save_to_disk()");
        return;
    }
