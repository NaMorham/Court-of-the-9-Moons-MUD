 ...
 * define elsewhere.
 */
#ifndef __TBAMUD__UTILS_H__     // Begin header file protection
#define __TBAMUD__UTILS_H__
...
const int   dice(const int number, const int size);
...
extern const int MIN(const int a, const int b);
extern const int LIMIT(const int val, const int minv, const int maxv);
char *CAP(char *txt);
...
/**
 * If string begins a vowel (upper or lower case), return "an"; else return "a".
 */
...
#define CREATE(result, type, number)  do {                                               \
    if ((number) * sizeof(type) <= 0)                                                    \
        WriteLogf("SYSERR: Zero bytes or less requested at %s:%d.", __FILE__, __LINE__); \
 ...
 * room utils
 */
// Return the sector type for the room. If there is no sector type, return SECT_INSIDE.
...
#define GET_STR(ch)             ((ch)->aff_abils.getStr())
// Current strength modifer of ch.
#define GET_ADD(ch)             ((ch)->aff_abils.getStrAdd())
// Current dexterity of ch.
#define GET_DEX(ch)             ((ch)->aff_abils.getDex())
// Current intelligence of ch.
#define GET_INT(ch)             ((ch)->aff_abils.getIntel())
// Current wisdom of ch.
#define GET_WIS(ch)             ((ch)->aff_abils.getWis())
// Current constitution of ch.
#define GET_CON(ch)             ((ch)->aff_abils.getCon())
// Current charisma of ch.
#define GET_CHA(ch)             ((ch)->aff_abils.getCha())
...
// Defines if sub character can see obj character, assuming mortal only settings.
#define MORT_CAN_SEE(sub, obj) (LIGHT_OK(sub) && INVIS_OK(sub, obj))

// Defines if sub character can see obj character, assuming immortal and mortal settings.
...
// If vict can see obj, return obj short description, else return "something".
 ...
 */
/*
inline const char *RaceAbbrev(struct char_data *ch) {
    if (ch && (GET_RACE(ch) >= 0 && GET_RACE(ch) < NUM_RACES)) {
        return race_abbrevs[GET_RACE(ch)];
    }
    else {
        return "--";
    }
...
/**
 * true if ch is a Trolloc, false if not.
 */
inline const bool isTrolloc(struct char_data *ch)
{
    return (GET_RACE(ch) == RACE_TROLLOC);
}

/**
 * true if ch is a Human, false if not.
 */
inline const bool isHuman(struct char_data *ch)
{
    return (GET_RACE(ch) == RACE_HUMAN);
}

/**
 * true if ch is an Ogier, false if not.
 */
inline const bool isOgier(struct char_data *ch)
{
    return (GET_RACE(ch) == RACE_OGIER);
}

/**
 * true if ch is a Fade, false if not.
 */
