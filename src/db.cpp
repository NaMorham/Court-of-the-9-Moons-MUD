...
static int fileToString(const char *name, std::string &str);
...
static void boot_social_messages(void)
{
    ...
    WriteLogf("Social table contains %d socials.", top_of_socialt);
    rewind(fl);
    ...
    for (line_number = 0;; ++line_number) {
        ...
        if (CONFIG_NEW_SOCIALS == TRUE) {
            if (fscanf(fl, " %s %d %d %d %d \n",
                ...
                WriteLogf("SYSERR: format error in social file near social '%s'", next_soc);
...
#ifdef CIRCLE_ACORN
        if (fgetc(fl) != '\n') {
            WriteLogf("SYSERR: Acorn bug workaround failed.");
        }
...
{
    WriteLogf("Loading zone table.");
    index_boot(DB_BOOT_ZON);

    WriteLogf("Loading triggers and generating index.");
    index_boot(DB_BOOT_TRG);

    WriteLogf("Loading rooms.");
    index_boot(DB_BOOT_WLD);

    WriteLogf("Renumbering rooms.");
    renum_world();

    WriteLogf("Checking start rooms.");
    check_start_rooms();

    WriteLogf("Loading mobs and generating index.");
    index_boot(DB_BOOT_MOB);

    WriteLogf("Loading objs and generating index.");
    index_boot(DB_BOOT_OBJ);

    WriteLogf("Renumbering zone table.");
    renum_zone_table();

    if (converting) {
        WriteLogf("Saving 128bit world files to disk.");
        save_all();
    }

    if (!no_specials) {
        WriteLogf("Loading shops.");
        index_boot(DB_BOOT_SHP);
    }

    WriteLogf("Loading quests.");
    ...
    for (cnt = 0; cnt <= top_of_world; cnt++) {
        ...
        for (itr = 0; itr < NUM_OF_DIRS; itr++) { // NUM_OF_DIRS here, not DIR_COUNT
            ...
            if (world[cnt].dir_option[itr]->general_description) {
                ...
                world[cnt].dir_option[itr]->general_description = NULL;
            }
...
void boot_db(void)
{
    ...
    WriteLogf("Loading spell definitions.");
    mag_assign_spells();

    boot_world();

    WriteLogf("Loading help entries.");
    index_boot(DB_BOOT_HLP);

    WriteLogf("Generating player index.");
    build_player_index();

    if (auto_pwipe) {
        WriteLogf("Cleaning out inactive pfiles.");
        clean_pfiles();
    }

    WriteLogf("Loading fight messages.");
    load_messages();

    WriteLogf("Loading social messages.");
    boot_social_messages();

    WriteLogf("Building command list.");
    create_command_list(); // aedit patch -- M. Scott

    WriteLogf("Assigning function pointers:");

    if (!no_specials) {
        WriteLogf("   Mobiles.");
        assign_mobiles();
        WriteLogf("   Shopkeepers.");
    ...
    WriteLogf("Assigning spell and skill levels.");
    init_spell_levels();

    WriteLogf("Sorting command list and spells.");
    sort_commands();
    sort_spells();

    WriteLogf("Booting mail system.");
    if (!scan_file()) {
        WriteLogf("    Mail boot failed -- Mail system disabled");
        no_mail = 1;
    }
    WriteLogf("Reading banned site and invalid-name list.");
    load_banned();
    read_invalid_list();

    WriteLogf("Loading Ideas.");
    load_ibt_file(SCMD_IDEA);

    WriteLogf("Loading Bugs.");
    load_ibt_file(SCMD_BUG);

    WriteLogf("Loading Typos.");
    load_ibt_file(SCMD_TYPO);

    if (!no_rent_check) {
        WriteLogf("Deleting timed-out crash and rent files:");
        update_obj_file();
        WriteLogf("   Done.");
    }
    ...
    if (!mini_mud)  {
        WriteLogf("Booting houses.");
        House_boot();
    }

    WriteLogf("Cleaning up last log.");
    clean_llog_entries();
...
#if 1
    {
        ...
        for (j = 0; j < top_of_objt; j++) {
            if (obj_proto[j].script == (struct script_data *)&shop_keeper) {
                WriteLogf("Item %d (%s) had shopkeeper trouble.", obj_index[j].vnum, obj_proto[j].short_description);
    ...
    for (i = 0; i <= top_of_zone_table; i++) {
        WriteLogf("Resetting #%d: %s (rooms %d-%d).", zone_table[i].number,
...
static void reset_time(void)
{
    ...
    WriteLogf("   Current Gametime: %dH %dD %dM %dY.", time_info.hours,
...
void save_mud_time(struct time_info_data *when)
{
    ...
    if ((bgtime = fopen(TIME_FILE, "w")) == NULL) {
        WriteLogf("SYSERR: Can't write to '%s' time file.", TIME_FILE);
    }
    else {
...
ackeof:
    WriteLogf("SYSERR: Unexpected end of help file.");
...
void index_boot(int mode)
{
    ...
    case DB_BOOT_QST:
        ...
        break;
    default:
        WriteLogf("SYSERR: Unknown subcommand %d to index_boot!", mode);
        exit(1);
    }
    ...
    if (!(db_index = fopen(buf2, "r"))) {
        WriteLogf("SYSERR: opening index file '%s': %s", buf2, strerror(errno));
        exit(1);
    }
    ...
    for (line_number = 0;; ++line_number) {
        ...
        if (!(db_file = fopen(buf2, "r"))) {
            WriteLogf("SYSERR: File '%s' listed in '%s/%s': %s", buf2, prefix, index_filename, strerror(errno));
        }
        else {
            if (mode == DB_BOOT_ZON) {
                rec_count++;
    ...
    case DB_BOOT_WLD:
        ...
        WriteLogf("   %d rooms, %d bytes.", rec_count, size[0]);
        break;
    case DB_BOOT_MOB:
        ...
        WriteLogf("   %d mobs, %d bytes in index, %d bytes in prototypes.", rec_count, size[0], size[1]);
        break;
    case DB_BOOT_OBJ:
        ...
        WriteLogf("   %d objs, %d bytes in index, %d bytes in prototypes.", rec_count, size[0], size[1]);
        break;
    case DB_BOOT_ZON:
        CREATE(zone_table, struct zone_data, rec_count);
        size[0] = sizeof(struct zone_data) * rec_count;
        WriteLogf("   %d zones, %d bytes.", rec_count, size[0]);
        break;
    case DB_BOOT_HLP:
        CREATE(help_table, struct help_index_element, rec_count);
        size[0] = sizeof(struct help_index_element) * rec_count;
        WriteLogf("   %d entries, %d bytes.", rec_count, size[0]);
        break;
    case DB_BOOT_QST:
    ...
    for (line_number = 1;; ++line_number) {
        ...
        snprintf(buf2, sizeof(buf2), "%s%s", prefix, buf1);
        if (!(db_file = fopen(buf2, "r"))) {
            WriteLogf("SYSERR: %s: %s", buf2, strerror(errno));
            exit(1);
        }
        switch (mode) {
        case DB_BOOT_WLD:
...
void discrete_load(FILE *fl, int mode, char *filename)
{
    ...
    for (;;) {
        ...
        if ((mode != DB_BOOT_OBJ) || (nr < 0)) {
            if (!get_line(fl, line)) {
                if (nr == -1) {
                    WriteLogf("SYSERR: %s file %s is empty!", modes[mode], filename);
                }
                else {
                    WriteLogf("SYSERR: Format error in %s after %s #%d\n"
        ...
        if (*line == '#') {
            ...
            if (sscanf(line, "#%d", &nr) != 1) {
                WriteLogf("SYSERR: Format error after %s #%d", modes[mode], last);
                exit(1);
            }
            if (nr >= 99999) {
                return;
            ...
            WriteLogf("SYSERR: Format error in %s file %s near %s #%d", modes[mode],
                filename, modes[mode], nr);
            WriteLogf("SYSERR: ... offending line: '%s'", line);
            exit(1);
        }
    }  // for (;;)
}
...
void parse_room(FILE *fl, int virtual_nr)
{
    ...
    if (virtual_nr < zone_table[zone].bot) {
        WriteLogf("SYSERR: Room #%d is below zone %d (bot=%d, top=%d).", virtual_nr, zone_table[zone].number, zone_table[zone].bot, zone_table[zone].top);
        exit(1);
    }
    while (virtual_nr > zone_table[zone].top) {
        if (++zone > top_of_zone_table) {
            WriteLogf("SYSERR: Room %d is outside of any zone.", virtual_nr);
            exit(1);
        }
    }
    ...
    if (!get_line(fl, line)) {
        WriteLogf("SYSERR: Expecting roomflags/sector type of room #%d but file ended!",
            virtual_nr);
        exit(1);
    }

    if (((retval = sscanf(line, " %d %s %s %s %s %d ", t, flags, flags2, flags3, flags4, t + 2)) == 3) && (bitwarning == TRUE)) {
        WriteLogf("WARNING: Conventional world files detected. See config.c.");
        exit(1);
    }
        ...
        // is converted.
        WriteLogf("Converting room #%d to 128bits..", virtual_nr);
        ...
        WriteLogf("SYSERR: Format error in roomflags/sector type of room #%d", virtual_nr);
        exit(1);
    }
...
void setup_dir(FILE *fl, int room, int dir)
{
    ...
    if (!get_line(fl, line)) {
        WriteLogf("SYSERR: Format error, %s", buf2);
        exit(1);
    }
...
static void check_start_rooms(void)
{
    if ((r_mortal_start_room = real_room(CONFIG_MORTAL_START)) == NOWHERE) {
        WriteLogf("SYSERR:  Mortal start room does not exist.  Change in config.c.");
        exit(1);
    }
    if ((r_immort_start_room = real_room(CONFIG_IMMORTAL_START)) == NOWHERE) {
        if (!mini_mud) {
            WriteLogf("SYSERR:  Warning: Immort start room does not exist.  Change in config.c.");
        }
        r_immort_start_room = r_mortal_start_room;
    }
    if ((r_frozen_start_room = real_room(CONFIG_FROZEN_START)) == NOWHERE) {
        if (!mini_mud) {
            WriteLogf("SYSERR:  Warning: Frozen start room does not exist.  Change in config.c.");
        }
...
static void parse_simple_mob(FILE *mob_f, int i, int nr)
{
    ...
    mob_proto[i].real_abils.setStr(11);
    mob_proto[i].real_abils.setIntel(11);
    mob_proto[i].real_abils.setWis(11);
    mob_proto[i].real_abils.setDex(11);
    mob_proto[i].real_abils.setCon(11);
    mob_proto[i].real_abils.setCha(11);

    if (!get_line(mob_f, line)) {
        WriteLogf("SYSERR: Format error in mob #%d, file ended after S flag!", nr);
        exit(1);
    }

    if (sscanf(line, " %d %d %d %dd%d+%d %dd%d+%d ",
        t, t + 1, t + 2, t + 3, t + 4, t + 5, t + 6, t + 7, t + 8) != 9) {
        WriteLogf("SYSERR: Format error in mob #%d, first line after S flag\n"
    ...
    if (!get_line(mob_f, line)) {
        WriteLogf("SYSERR: Format error in mob #%d, second line after S flag\n"
    ...
    if (!get_line(mob_f, line)) {
        WriteLogf("SYSERR: Format error in last line of mob #%d\n"
            "...expecting line of form '# # #', but file ended!", nr);
        exit(1);
    }
    ...
    GET_CLASS(mob_proto + i) = 0;  // @todo: use constant value
    GET_RACE(mob_proto + i) = RACE_HUMAN;
...
static void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
    ...
    if (!matched) {
        WriteLogf("SYSERR: Warning: unrecognized espec keyword %s in mob #%d", keyword, nr);
    }
}
#undef CASE
#undef BOOL_CASE
...
static void parse_enhanced_mob(FILE *mob_f, int i, int nr)
{
    ...
    while (get_line(mob_f, line)) {
        if (!strcmp(line, "E")) {  // end of the enhanced section */
            ...
            WriteLogf("SYSERR: Unterminated E section in mob #%d", nr);
            exit(1);
        }
        else {
    ...
    WriteLogf("SYSERR: Unexpected end of file reached after mob #%d", nr);
    exit(1);
}
...
void parse_mobile(FILE *mob_f, int nr)
{
    ...
    if (MOB_FLAGGED(mob_proto + i, MOB_NOTDEADYET)) {
        ...
        WriteLogf("SYSERR: Mob #%d has reserved bit MOB_NOTDEADYET set.", nr);
    ...
    default:
        WriteLogf("SYSERR: Unsupported mob type '%c' in mob #%d", letter, nr);
        exit(1);
    }
...
char *parse_object(FILE *obj_f, int nr)
{
    ...
    if ((obj_proto[i].name = fread_string(obj_f, buf2)) == NULL) {
        WriteLogf("SYSERR: Null obj name or format error at or near %s", buf2);
        exit(1);
    }
    ...
    if (!get_line(obj_f, line)) {
        WriteLogf("SYSERR: Expecting first numeric line of %s, but file ended!", buf2);
        exit(1);
    }
    ...
    if (((retval = sscanf(line, " %d %s %s %s %s %s %s %s %s %s %s %s %s", t, f1, f2, f3,
        ...
        // Let's make the implementor read some, before converting his world files.
        WriteLogf("WARNING: Conventional object files detected. Please see config.c.");
        exit(1);
    }
        ...
        WriteLogf("Converting object #%d to 128bits..", nr);
        ...
        WriteLogf("SYSERR: Format error in first numeric line (expecting 13 args, got %d), %s", retval, buf2);
        exit(1);
    }
    ...
    if (!get_line(obj_f, line)) {
        WriteLogf("SYSERR: Expecting second numeric line of %s, but file ended!", buf2);
        exit(1);
    }
    if ((retval = sscanf(line, "%d %d %d %d", t, t + 1, t + 2, t + 3)) != 4) {
        WriteLogf("SYSERR: Format error in second numeric line (expecting 4 args, got %d), %s", retval, buf2);
        exit(1);
    }
    ...
    if (!get_line(obj_f, line)) {
        WriteLogf("SYSERR: Expecting third numeric line of %s, but file ended!", buf2);
        exit(1);
    }
    if ((retval = sscanf(line, "%d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4)) != 5) {
        if (retval == 3) {
            ...
            WriteLogf("SYSERR: Format error in third numeric line (expecting 4 args, got %d), %s", retval, buf2);
            exit(1);
        }
    }
    ...
    for (;;) {
        if (!get_line(obj_f, line)) {
            WriteLogf("SYSERR: Format error in %s", buf2);
            exit(1);
        }
        switch (*line) {
        case 'E':
        ...
        case 'A':
            if (j >= MAX_OBJ_AFFECT) {
                WriteLogf("SYSERR: Too many A fields (%d max), %s", MAX_OBJ_AFFECT, buf2);
                exit(1);
            }
            if (!get_line(obj_f, line)) {
                WriteLogf("SYSERR: Format error in 'A' field, %s\n"
        ...
        case '#':
            ...
            return (line);
        default:
            WriteLogf("SYSERR: Format error in (%c): %s", *line, buf2);
            exit(1);
        }
    }  // for (;;)
}

#define ZTBLE    zone_table[zone]
/*
...
static void load_zones(FILE *fl, char *zonename)
{
    ...
    int i, cmd_no, tmp, error;
    int num_of_cmds = 0;
    int line_num = 0;
    int zone_fix = FALSE;
    char *ptr;
    char zname[READ_SIZE];
    char buf[READ_SIZE], buf2[MAX_STRING_LENGTH];
    char t1[80], t2[80];
    ...
    if (num_of_cmds == 0) {
        WriteLogf("SYSERR: %s is empty!", zname);
        exit(1);
    }
    else {
    ...
    snprintf(buf2, sizeof(buf2), "beginning of zone #%d", ZTBLE.number);

    line_num += get_line(fl, buf);
    if ((ptr = strchr(buf, '~')) != NULL) {  // take off the '~' if it's there
        *ptr = '\0';
    }
    ZTBLE.builders = strdup(buf);
    ...
    if (sscanf(buf, " %hd %hd %d %d %s %s %s %s %d %d", &ZTBLE.bot, &ZTBLE.top, &ZTBLE.lifespan,
        ...
        if (sscanf(buf, " %hd %hd %d %d ", &ZTBLE.bot, &ZTBLE.top, &ZTBLE.lifespan, &ZTBLE.reset_mode) != 4) {
            ...
            if (sscanf(ZTBLE.name, " %hd %hd %d %d ", &ZTBLE.bot, &ZTBLE.top, &ZTBLE.lifespan, &ZTBLE.reset_mode) != 4) {
                ...
                free(ZTBLE.name);
                ZTBLE.name = strdup(ZTBLE.builders);
                free(ZTBLE.builders);
                ZTBLE.builders = strdup("None.");
                zone_fix = TRUE;
            }
        }
        ...
        ZTBLE.zone_flags[0] = asciiflag_conv(zbuf1);
        ZTBLE.zone_flags[1] = asciiflag_conv(zbuf2);
        ZTBLE.zone_flags[2] = asciiflag_conv(zbuf3);
    ...
    for (;;) {
        ...
        if (zone_fix != TRUE) {
            if ((tmp = get_line(fl, buf)) == 0) {
                WriteLogf("SYSERR: Format error in %s - premature end of file", zname);
                exit(1);
            }
        }
        else {
        ...
        if (strchr("MOGEPDTV", ZCMD.command) == NULL) {  // a 3-arg command
            ...
            if (sscanf(ptr, " %d %d %d %d %79s %79[^\f\n\r\t\v]", &tmp, &ZCMD.arg1, &ZCMD.arg2, &ZCMD.arg3, t1, t2) != 6) {
                error = 1;
            }
            else {
    ...
    if (num_of_cmds != (cmd_no + 1)) {
        WriteLogf("SYSERR: Zone command count mismatch for %s. Estimated: %d, Actual: %d", zname, num_of_cmds, cmd_no + 1);
        exit(1);
    }
...
static void get_one_line(FILE *fl, char *buf)
{
    if (fgets(buf, READ_SIZE, fl) == NULL) {
        WriteLogf("SYSERR: error reading help file: not terminated with $?");
        exit(1);
    }
...
void load_help(FILE * fl, char *name)
{
    ...
    while (*key != '$') {
        ...
        if (entrylen >= sizeof(entry) - 1) {
            ...
            WriteLogf("SYSERR: Help entry exceeded buffer space: %.*s", keysize, key);
        ...
        if (*line == '#') {
            if (sscanf(line, "#%d", &el.min_level) != 1) {
                WriteLogf("SYSERR: Help entry does not have a min level. %s", key);
                el.min_level = 0;
            }
        }
...
struct char_data *read_mobile(mob_vnum nr, int type) // and mob_rnum
{
    ...
    if (type == VIRTUAL) {
        if ((i = real_mobile(nr)) == NOBODY) {
            WriteLogf("WARNING: Mobile vnum %d does not exist in database.", nr);
            return (NULL);
        }
    }
    else {
...
struct obj_data *read_object(obj_vnum nr, int type) // and obj_rnum
{
    ...
    if ((i == NOTHING) || (i > top_of_objt)) {
        WriteLogf("Object (%c) %d does not exist in database.", type == VIRTUAL ? 'V' : 'R', nr);
        return (NULL);
    }
    ...
    for (update_u = reset_q.head; update_u; update_u = update_u->next) {
        if (zone_table[update_u->zone_to_reset].reset_mode == 2 || is_empty(update_u->zone_to_reset)) {
            ...
            if (update_u == reset_q.head) {
                ...
                for (temp = reset_q.head; temp->next != update_u; temp = temp->next) { } // do nothing, this just finds the zone for update
                if (!update_u->next) {
                    reset_q.tail = temp;
                }
...
char *fread_string(FILE *fl, const char *error)
{
    ...
    *buf = '\0';
    ...
    do {
        if (!fgets(tmp, 512, fl)) {
            WriteLogf("SYSERR: fread_string: format error at or near %s", error);
            exit(1);
        }
        ...
        if (length + templength >= MAX_STRING_LENGTH) {
            WriteLogf("SYSERR: fread_string: string too large (db.c)");
            WriteLogf("%s", error);
            exit(1);
        }
        else {
...
void free_char(struct char_data *ch)
{
    ...
    if (ch->player_specials != NULL && ch->player_specials != &dummy_mob) {
        ...
        if (IS_NPC(ch)) {
            WriteLogf("SYSERR: Mob %s (#%d) had player_specials allocated!", GET_NAME(ch), GET_MOB_VNUM(ch));
        }
    }
...
static int file_to_string(const char *name, char *buf)
{
    ...
    *buf = '\0';
    ...
    if (!(fl = fopen(name, "r"))) {
        WriteLogf("SYSERR: reading %s: %s", name, strerror(errno));
        return (-1);
    }
    ...
    for (;;) {
        ...
        if (strlen(buf) + strlen(tmp) + 1 > MAX_STRING_LENGTH) {
            WriteLogf("SYSERR: %s: string too big (%d max)", name, MAX_STRING_LENGTH);
            *buf = '\0';
            fclose(fl);
            return (-1);
        }
...
void init_char(struct char_data *ch)
{
    ...
    // @todo:  Apply racial shift to height and weight ranges
    ...
    if (GET_SEX(ch) == SEX_MALE) {
        ...
        GET_HEIGHT(ch) = rand_number(160, 200);     // 5'4" - 6'8" -- for people who can't do metric
    }
    else {
        GET_WEIGHT(ch) = rand_number(100, 160);
        GET_HEIGHT(ch) = rand_number(150, 180);     // 5'0" - 6'0" -- for people who can't do metric
    }
    ...
    if ((i = get_ptable_by_name(GET_NAME(ch))) != -1) {
        ...
        WriteLogf("SYSERR: init_char: Character '%s' not found in player table.", GET_NAME(ch));
    }
    ...
    ch->real_abils.setIntel(25);
    ch->real_abils.setWis(25);
    ch->real_abils.setDex(25);
    ch->real_abils.setStr(25);
    ch->real_abils.setStrAdd(100);
    ch->real_abils.setCon(25);
    ch->real_abils.setCha(25);
...
static int check_object(struct obj_data *obj)
{
    ...
    if ((GET_OBJ_WEIGHT(obj) < 0) && (error = TRUE)) {
        WriteLogf("SYSERR: Object #%d (%s) has negative weight (%d).",
            GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_WEIGHT(obj));
    }

    if ((GET_OBJ_RENT(obj) < 0) && (error = TRUE)) {
        WriteLogf("SYSERR: Object #%d (%s) has negative cost/day (%d).",
    ...
    case ITEM_DRINKCON:
    {
        ...
        if (search_block(onealias, drinknames, TRUE) < 0 && (error = TRUE)) {
            WriteLogf("SYSERR: Object #%d (%s) doesn't have drink type as last keyword. (%s)",
    ...
    case ITEM_FOUNTAIN:
        if ((GET_OBJ_VAL(obj, 0) > 0) && (GET_OBJ_VAL(obj, 1) > GET_OBJ_VAL(obj, 0)) && (error = TRUE)) {
            WriteLogf("SYSERR: Object #%d (%s) contains (%d) more than maximum (%d).",
    ...
    case ITEM_STAFF:
        ...
        if (GET_OBJ_VAL(obj, 2) > GET_OBJ_VAL(obj, 1) && (error = TRUE)) {
            WriteLogf("SYSERR: Object #%d (%s) has more charges (%d) than maximum (%d).",
                GET_OBJ_VNUM(obj), obj->short_description,
        ...
        if (obj->ex_description) {
            ...
            do {
                if (find_exdesc(onealias, obj->ex_description) && (error = TRUE)) {
                    WriteLogf("SYSERR: Object #%d (%s) is type NOTE and has extra description with same name. (%s)",
                        GET_OBJ_VNUM(obj), obj->short_description, obj->name);
                }
...
static int check_object_spell_number(struct obj_data *obj, int val)
{
    ...
    if (error) {
        WriteLogf("SYSERR: Object #%d (%s) has out of range spell #%d.",
    ...
    if (((spellname == unused_spellname) || !str_cmp("UNDEFINED", spellname)) && (error = TRUE)) {
        WriteLogf("SYSERR: Object #%d (%s) uses '%s' spell #%d.",
...
static int check_object_level(struct obj_data *obj, int val)
{
    ...
    if (((GET_OBJ_VAL(obj, val) < 0) || (GET_OBJ_VAL(obj, val) > LVL_IMPL)) && (error = TRUE)) {
        WriteLogf("SYSERR: Object #%d (%s) has out of range level #%d.",
            GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_VAL(obj, val));
    }
...
static int check_bitvector_names(bitvector_t bits, size_t namecount, const char *whatami, const char *whatbits)
{
    ...
    for (flagnum = namecount; flagnum < sizeof(bitvector_t) * 8; flagnum++) {
        if ((1 << flagnum) & bits) {
            WriteLogf("SYSERR: %s has unknown %s flag, bit %d (0 through %d known).", whatami, whatbits, flagnum, (int)namecount - 1);
            error = TRUE;
        }
    }  // for (flagnum ...
    ...
    while (get_line(fl, line)) {
        ...
        case 'a':
            if (!str_cmp(tag, "auto_save")) {
                ...
                WriteLogf("WARNING: Unknown 'a' tag: %s", tag);
            }
            break;

        case 'b':
            // No known B tags
            WriteLogf("WARNING: Unknown 'b' tag: %s", tag);
        ...
        case 'c':
            if (!str_cmp(tag, "crash_file_timeout")) {
                ...
                WriteLogf("WARNING: Unknown 'c' tag: %s", tag);
            }
            break;
        ...
        case 'd':
            if (!str_cmp(tag, "debug_mode"))
                ...
                WriteLogf("WARNING: Unknown 'd' tag: %s", tag);
            }
            break;

        case 'e':
            // No known E tags
            WriteLogf("WARNING: Unknown 'e' tag: %s", tag);
        ...
        case 'f':
            if (!str_cmp(tag, "free_rent")) {
                ...
                WriteLogf("WARNING: Unknown 'f' tag: %s", tag);
            }
            break;

        case 'g':
            // No known G tags
            WriteLogf("WARNING: Unknown 'g' tag: %s", tag);
        ...
        case 'h':
            if (!str_cmp(tag, "holler_move_cost")) {
                ...
                WriteLogf("WARNING: Unknown 'h' tag: %s", tag);
            }
            break;
        ...
        case 'i':
            if (!str_cmp(tag, "idle_void")) {
                ...
                WriteLogf("WARNING: Unknown 'i' tag: %s", tag);
            }
            break;

        case 'j':
            // No known J tags
            WriteLogf("WARNING: Unknown 'j' tag: %s", tag);

        case 'k':
            // No known K tags
            WriteLogf("WARNING: Unknown 'k' tag: %s", tag);
        ...
        case 'l':
            if (!str_cmp(tag, "level_can_shout")) {
                ...
                WriteLogf("WARNING: Unknown 'l' tag: %s", tag);
            }
            break;
        ...
        case 'm':
            if (!str_cmp(tag, "max_bad_pws")) {
                ...
                WriteLogf("WARNING: Unknown 'm' tag: %s", tag);
            }
            break;
        ...
        case 'n':
            if (!str_cmp(tag, "nameserver_is_slow")) {
                ...
                WriteLogf("WARNING: Unknown 'n' tag: %s", tag);
            }
            break;
        ...
        case 'o':
            if (!str_cmp(tag, "ok")) {
                ...
                WriteLogf("WARNING: Unknown 'o' tag: %s", tag);
            }
            break;
        ...
        case 'p':
            if (!str_cmp(tag, "pk_allowed")) {
                ...
                WriteLogf("WARNING: Unknown 'p' tag: %s", tag);
            }
            break;

        case 'q':
            // No known Q tags
            WriteLogf("WARNING: Unknown 'q' tag: %s", tag);
        ...
        case 'r':
            if (!str_cmp(tag, "rent_file_timeout")) {
                ...
                WriteLogf("WARNING: Unknown 'r' tag: %s", tag);
            }
            break;
        ...
        case 's':
            if (!str_cmp(tag, "siteok_everyone")) {
                ...
                WriteLogf("WARNING: Unknown 's' tag: %s", tag);
            }
            break;
        ...
        case 't':
            if (!str_cmp(tag, "tunnel_size")) {
                ...
                WriteLogf("WARNING: Unknown 't' tag: %s", tag);
            }
            break;
        ...
        case 'u':
            if (!str_cmp(tag, "use_autowiz")) {
                ...
                WriteLogf("WARNING: Unknown 'u' tag: %s", tag);
            }
            break;

        case 'v':
            // No known V tags
            WriteLogf("WARNING: Unknown 'v' tag: %s", tag);
        ...
        case 'w':
            if (!str_cmp(tag, "welc_messg")) {
                ...
                WriteLogf("WARNING: Unknown 'w' tag: %s", tag);
            }
            break;

        case 'x':
            // No known X tags
            WriteLogf("WARNING: Unknown 'x' tag: %s", tag);
            break;

        case 'y':
            // No known Y tags
            WriteLogf("WARNING: Unknown 'y' tag: %s", tag);
            break;

        case 'z':
            // No known Z tags
            WriteLogf("WARNING: Unknown 'z' tag: %s", tag);
            break;

        default:
            // NFC what tag
            WriteLogf("SYSERR: Unknown tag: %s", tag);
            break;
        }
    }
