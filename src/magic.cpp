    ...
    IDXTYPE item1, IDXTYPE item2, int extract, int verbose)
{
    ...
    // If we needed items, but didn't find all of them, then the spell is a failure.
    ...
    int spellnum, int savetype)
{
    ...
    case SPELL_STRENGTH:
        if (victim->aff_abils.getStrAdd() == 100) {
            // cannot make them stronger
            return;
        }
     ...
     int spellnum, int type)
{
    ...
    case SPELL_REMOVE_CURSE:
        ...
        break;
    default:
        WriteLogf("SYSERR: unknown spellnum %d passed to mag_unaffects.", spellnum);
        return;
    }
...
void mag_creations(int level, struct char_data *ch, int spellnum)
{
    ...
    if (!(tobj = read_object(z, VIRTUAL))) {
        ...
        WriteLogf("SYSERR: spell_creations, spell %d, obj %d: obj not found", spellnum, z);
        return;
    }
    obj_to_char(tobj, ch);
