...
#endif
        WriteLogf("SYSERR: GenOLC: save_objects: Invalid real zone number %d. (0-%d)", zone_num, top_of_zone_table);
        return FALSE;
    }
...
int delete_object(obj_rnum rnum)
{
    ...
    WriteLogf("GenOLC: delete_object: Deleting object #%d (%s).", GET_OBJ_VNUM(obj), obj->short_description);
    ...
    for (zone = 0; zone <= top_of_zone_table; zone++) {
        for (cmd_no = 0; ZCMD(zone, cmd_no).command != 'S'; cmd_no++) {
            ...
            case 'R':
                ...
                break;
            default:
                WriteLogf("GenOLC (obj): Unknown zone command (%c).", ZCMD(zone, cmd_no).command);
            }
        }  // for (cmd_no
    }  // for (zone ...
