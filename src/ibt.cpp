...
static IBT_DATA *read_ibt(char *filename, FILE *fp)
{
    ...
    for (;;)
    {
        ...
        default:
            WriteLogf("SYSERR: Invalid keyword (%s) in IBT file", word);
            break;
        }
        if (!fMatch)
        {
            fread_to_eol(fp);
            WriteLogf("read_ibt (%s): no match: %s - aborting line!", filename, word);
        }
    }
    WriteLogf("read_ibt: Aborted! Returning NULL");
    if (ibtData->name)
...
void load_ibt_file(int mode)
{
   ...
   switch(mode) {
     ...
     case SCMD_TYPO: sprintf( filename, "%s",TYPOS_FILE );
                     ...
                     last_ibt  = last_typo;
                     break;
     default: WriteLogf("SYSERR: Invalid mode (%d) in load_ibt_file", mode);
                     return;
   }
...
void save_ibt_file(int mode)
{
    ...
    {
        WriteLogf("SYSERR: Unable to open IBT file for writing in save_ibt_file");
        WriteLogf("        IBT File: %s", filename);
        return;
    }
    else
    {
...
static IBT_DATA *get_first_ibt(int mode)
{
    ...
    case SCMD_TYPO: first_ibt = first_typo;
        break;
    default: WriteLogf("SYSERR: Invalid mode (%d) in get_first_ibt", mode);
        break;
    }
    return (first_ibt);
}
...
static IBT_DATA *get_last_ibt(int mode)
{
    ...
    case SCMD_TYPO: last_ibt = last_typo;
        break;
    default: WriteLogf("SYSERR: Invalid mode (%d) in get_last_ibt", mode);
        break;
    }
    return (last_ibt);
}
...
ACMD(do_ibt)
{
    ...
    else if (is_abbrev(arg, "submit"))
    {
        ...
        case SCMD_TYPO: SET_BIT_AR(PLR_FLAGS(ch), PLR_TYPO);
            break;
        default: WriteLogf("Invalid subcmd (%d) in do_ibt", subcmd);
            return;
        }
...
static void ibtedit_save(struct descriptor_data *d)
{
    ...
    if ((ibtData = get_ibt_by_num(OLC_VAL(d), OLC_NUM(d))) != NULL) {
        ...
        WriteLogf("SYSERR: ibtedit_save: Invalid IBT vnum (%d) in OLC struct", OLC_NUM(d));
        WriteLogf("        IBT possibly removed while being edited");
        return;
    }
