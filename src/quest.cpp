...
void assign_the_quests(void)
{
    ...
    for (rnum = 0; rnum < total_quests; rnum++) {
        if (QST_MASTER(rnum) == NOBODY) {
            WriteLogf("SYSERR: Quest #%d has no questmaster specified.", QST_NUM(rnum));
            continue;
        }
...
struct obj_data *object, int type)
{
    ...
    case AQ_ROOM_CLEAR:
        ...
        break;
    default:
        WriteLogf("SYSERR: Invalid quest type passed to autoquest_trigger_check");
        break;
    }
}
