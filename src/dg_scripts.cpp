...
char *str_str(char *cs, char *ct)
{
    ...
    while (*cs) {
        ...
        // If there we haven reached the end of ct via t, then the whole string was found.
        if (!*t) {
            return s;
        }
    }
 ...
 *
 * @todo Should this function not be constrained to the same room as an object  if 'name' is a unique id?
 *
 ...
 *
 * @todo Should this function not be constrained to the room if 'name' is a unique id?
 *
...
obj_data *get_obj_by_obj(obj_data *obj, char *name)
{
    ...
    else if (!str_cmp(name, "self") || !str_cmp(name, "me")) {
        return obj;
    }
    else if (obj->contains && (i = get_obj_in_list(name, obj->contains))) {
        return i;
    }
    else if (obj->in_obj && isname(name, obj->in_obj->name)) {
        return obj->in_obj;
    }
    else if (obj->worn_by && (i = get_object_in_equip(obj->worn_by, name))) {
        return i;
...
static EVENTFUNC(trig_wait_event)
{
...
#if 1  // debugging
    {
        ...
        if (!found) {
            WriteLogf("Trigger restarted on unknown entity. Vnum: %d", GET_TRIG_VNUM(trig));
            WriteLogf("Type: %s trigger", type == MOB_TRIGGER ? "Mob" : type == OBJ_TRIGGER ? "Obj" : "Room");
            WriteLogf("attached %d places", trig_index[trig->nr]->number);
...
static void do_stat_trigger(struct char_data *ch, trig_data *trig)
{
    ...
    if (!trig) {
        WriteLogf("SYSERR: NULL trigger passed to do_stat_trigger.");
        return;
    }
...
ACMD(do_detach)
{
    ...
    if (!*arg1 || !*arg2) {
        send_to_char(ch, "Usage: detach [ mob | object | room ] { target } { trigger | 'all' }\r\n");
        return;
    }
...
void script_vlog(const char *format, va_list args)
{
    ...
    for (i = descriptor_list; i; i = i->next) {
        ...
        else if (GET_LEVEL(i->character) < LVL_BUILDER) {
            continue;
        }
        else if (PLR_FLAGGED(i->character, PLR_WRITING)) {
            continue;
        }
        else if (NRM > (PRF_FLAGGED(i->character, PRF_LOG1) ? 1 : 0) + (PRF_FLAGGED(i->character, PRF_LOG2) ? 2 : 0)) {
            continue;
        }
    ...
    struct script_data *sc, trig_data *trig)
{
    ...
    // TODO: replace with proper strip space functions
    // strip off extra spaces at begin and end - lhs
    ...
    // strip off extra spaces at begin and end - rhs
    ...
    trig_data *trig, int type)
{
    ...
    // valid operands, in order of priority each must also be defined in eval_op()
    static char *ops[] = {
        "||",
        "&&",
        "==",
...
static struct cmdlist_element *find_end(trig_data *trig, struct cmdlist_element *cl)
{
    ...
    for (c = cl->next; c; c = c->next) {
        for (p = c->cmd; *p && isspace(*p); p++) { }  // find the first non-space
...
static void process_remote(struct script_data *sc, trig_data *trig, char *cmd)
{
    ...
    if (sc_remote == NULL) {  // no script to assign
        return;
    }
...
static void process_rdelete(struct script_data *sc, trig_data *trig, char *cmd)
{
    ...
    if (sc_remote == NULL) {  // no script to delete a trigger from
        return;
    }
    if (sc_remote->global_vars == NULL) {  // no script globals
        return;
    }
    ...
    if (!vd) {  // the variable doesn't exist, or is the wrong context
        return;
    }
...
int script_driver(void *go_adress, trig_data *trig, int type, int mode)
{
    ...
    {
        for (p = cl->cmd; *p && isspace(*p); p++) { }  // loop through until we find a non space
    ...
    void *go, struct script_data *sc, int type, char *cond)
{
    ...
    for (c = cl->next; c->next; c = c->next) {
        for (p = c->cmd; *p && isspace(*p); p++) { }  // until we find a space
...
static struct cmdlist_element *find_done(struct cmdlist_element *cl)
{
    ...
    for (c = cl->next; c && c->next; c = c->next) {
        for (p = c->cmd; *p && isspace(*p); p++) { } // till we find a whitespace character
...
void save_char_vars(struct char_data *ch)
{
    ...
    // Note that currently, context will always be zero. This may change in the future.
    while (vars) {
...
static struct char_data *find_char_by_uid_in_lookup_table(long uid)
{
    ...
    for (; lt && lt->uid != uid; lt = lt->next) { }  // find the uid

    if (lt) {
        return (struct char_data *)(lt->c);
    }

    WriteLogf("find_char_by_uid_in_lookup_table : No entity with number %ld in lookup table", uid);
    return NULL;
}
...
static struct obj_data *find_obj_by_uid_in_lookup_table(long uid)
{
    ...
    WriteLogf("find_obj_by_uid_in_lookup_table : No entity with number %ld in lookup table", uid);
    return NULL;
}
...
void add_to_lookup_table(long uid, void *c)
{
    ...
    for (; lt->next; lt = lt->next) {
        if (lt->c == c && lt->uid == uid) {
            WriteLogf("Add_to_lookup failed. Already there. (uid = %ld)", uid);
            return;
        }
    }
...
void remove_from_lookup_table(long uid)
{
    ...
    if (flt)
    {
        for (lt = &lookup_table[bucket]; lt->next != flt; lt = lt->next) { } // nothing to do, we are searching the list above
    ...
    WriteLogf("remove_from_lookup. UID %ld not found.", uid);
}
