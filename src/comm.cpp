...
template<typename T>
void dumpArray(const char* name, T* pArray, size_t sz)
{
    std::stringstream ss;
    ss << "Array: [" << (name ? name : "Unknown") << "]" << std::endl;
    if ((sz < 1) || (pArray == nullptr)) {
        ss << " *** EMPTY *** " << std::endl;
    }
    else {
        for (size_t i = 0; i < sz; ++i) {
            ss << std::setw(3) << (i + 1) << "/" << sz << ": " << pArray[i] << std::endl;
        }
    }
    ss << std::endl;
#if defined (CIRCLE_WINDOWS)
...
int main(int argc, char **argv)
{
    ...
    while ((pos < argc) && (*(argv[pos]) == '-')) {
        if (*(argv[pos] + 1) == 'f') {
            if (*(argv[pos] + 2)) {
                CONFIG_CONFFILE = strdup(argv[pos] + 2);
            }
            else if (++pos < argc) {
                CONFIG_CONFFILE = strdup(argv[pos]);
            }
            else {
    ...
    while ((pos < argc) && (*(argv[pos]) == '-')) {
        ...
        case 'o':
            if (*(argv[pos] + 2)) {
                CONFIG_LOGNAME = strdup(argv[pos] + 2);
            }
            else if (++pos < argc) {
                CONFIG_LOGNAME = strdup(argv[pos]);
            }
            else {
    ...
    WriteLogf("Loading configuration.");
    WriteLogf("%s", tbamud_version);

    if (chdir(dir) < 0) {
        perror("SYSERR: Fatal error changing to data directory");
        return 1;
    }
    WriteLogf("Using %s as data directory.", dir);

    if (scheck) {
        boot_world();
    }
    else {
        WriteLogf("Running game on port %d.", port);
        init_game(port);
    }

    WriteLogf("Clearing game world.");
    destroy_db();

    if (!scheck) {
        WriteLogf("Clearing other memory.");
        free_bufpool();         // comm.c
...
void copyover_recover()
{
    ...
    WriteLogf("Copyover recovery initiated");

    fp = fopen(COPYOVER_FILE, "r");
    // there are some descriptors open which will hang forever then?
    if (!fp) {
        perror("copyover_recover:fopen");
        WriteLogf("Copyover file not found. Exitting.\n\r");
        exit(1);
    }
...
static void init_game(ush_int local_port)
{
    ...
    WriteLogf("Finding player limit.");
    max_players = get_max_players();

    // If copyover mother_desc is already set up
    if (!fCopyOver) {
        WriteLogf("Opening mother connection.");
    ...
    WriteLogf("Entering game loop.");

    game_loop(mother_desc);

    Crash_save_all();

    WriteLogf("Closing all sockets.");
    ...
    WriteLogf("Saving current MUD time.");
    ...
    WriteLogf("Normal termination of game.");
}
...
static socket_t init_socket(ush_int local_port)
{
...
#ifdef CIRCLE_WINDOWS
    {
        ...
        if (WSAStartup(wVersionRequested, &wsaData) != 0) {
            WriteLogf("SYSERR: WinSock not available!");
            exit(1);
        }
        ...
        WriteLogf("Max players set to %d", max_players);

        if ((s = socket(PF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
            WriteLogf("SYSERR: Error opening network connection: Winsock error #%d",
                WSAGetLastError());
            exit(1);
        }
    }
...
# endif
    ...
    if (max_descs <= 0) {
        WriteLogf("SYSERR: Non-positive max player limit!  (Set at %d using %s).", max_descs, method);
        exit(1);
    }
    WriteLogf("   Setting player limit to %d using %s.", max_descs, method);
    return (max_descs);
#endif // CIRCLE_UNIX
}
    ...
    while (!circle_shutdown) {
        ...
        if (descriptor_list == NULL) {
            ...
            if (select(local_mother_desc + 1, &input_set, (fd_set *)0, (fd_set *)0, NULL) < 0) {
                ...
                WriteLogf("New connection.  Waking up.");
            }
            gettimeofday(&last_time, (struct timezone *) 0);
        }
        ...
        if (missed_pulses <= 0) {
            WriteLogf("SYSERR: **BAD** MISSED_PULSES NONPOSITIVE (%d), TIME GOING BACKWARDS!!", missed_pulses);
            missed_pulses = 1;
        }

        // If we missed more than 30 seconds worth of pulses, just do 30 secs
        if (missed_pulses > 30 RL_SEC) {
            WriteLogf("SYSERR: Missed %d seconds worth of pulses.", missed_pulses / PASSES_PER_SEC);
...
static void record_usage(void)
{
    ...
    WriteLogf("nusage: %-3d sockets connected, %-3d sockets playing", sockets_connected, sockets_playing);
...
#ifdef RUSAGE    // Not RUSAGE_SELF because it doesn't guarantee prototype.
    {
        ...
        WriteLogf("rusage: user time: %ld sec, system time: %ld sec, max res size: %ld",
...
#define A "\x1B["
char *ANSI[] = { "@", A"0m",A"0m",
    A"0;30m",A"0;34m",A"0;32m",A"0;36m",A"0;31m",A"0;35m",A"0;33m",A"0;37m",
    A"1;30m",A"1;34m",A"1;32m",A"1;36m",A"1;31m",A"1;35m",A"1;33m",A"1;37m",
    A"40m",A"44m",A"42m",A"46m",A"41m",A"45m",A"43m",A"47m",
    A"5m",A"4m",A"1m",A"7m",
    "!"
};
...
ColourMapping colourMap[] = {
    // Start with escaped @ and reset codes
    { "@", '@' },   // escaped
    { A"0m", 'n' },  // normal/reset
    { A"0m", 'N' },  // normal/reset

    // Dark colours
    { A"0;30m", 'd' }, // dark (black)
    { A"0;34m", 'b' }, // blue
    { A"0;32m", 'g' }, // green
    { A"0;36m", 'c' }, // cyan
    { A"0;31m", 'r' }, // red
    ...
    // Bright colours
    { A"1;30m", 'D' }, // bold dark (black)
    { A"1;34m", 'B' }, // bold blue
    { A"1;32m", 'G' }, // bold green
    { A"1;36m", 'C' }, // bold cyan
    { A"1;31m", 'R' }, // bold red
    { A"1;35m", 'M' }, // bold magenta
    { A"1;33m", 'Y' }, // bold yellow
    { A"1;37m", 'W' }, // bold white

    // Backgrounds?
    { A"40m", '0' }, // dark (black) background
    { A"44m", '1' }, // blue background
    { A"42m", '2' }, // green background
    { A"46m", '3' }, // cyan background
    { A"41m", '4' }, // red background
    { A"45m", '5' }, // magenta background
    { A"43m", '6' }, // yellow background
    { A"47m", '7' }, // white background

    // Display/font/hilight
    { A"5m", 'l' }, // Slow blink
    { A"4m", 'u' }, // Underline
    { A"1m", 'o' }, // Bold
    { A"7m", 'e' }, // Reverse
...
static struct in_addr *get_bind_addr()
{
    ...
    if (bind_addr.s_addr == htonl(INADDR_ANY)) {
        WriteLogf("Binding to all IP interfaces on this host.");
    }
    else {
        WriteLogf("Binding only to IP address %s", inet_ntoa(bind_addr));
    }
...
int parse_ip(const char *addr, struct in_addr *inaddr)
{
    WriteLogf("SYSERR: warning: you're trying to set DFLT_IP but your system has no "
...
ssize_t perform_socket_write(socket_t desc, const char *txt, size_t length)
{
    ...
    if (result == 0) {
        ...
        WriteLogf("SYSERR: Huh??  write() returned 0???  Please report this!");
        return (-1);
    }
...
static ssize_t perform_socket_read(socket_t desc, char *read_point, size_t space_left)
{
    ...
    if (ret == 0) {
        WriteLogf("WARNING: EOF on socket read (connection broken by peer)");
        return (-1);
    }
...
static int process_input(struct descriptor_data *t)
{
    ...
    do {
        if (space_left <= 0) {
            WriteLogf("WARNING: process_input: about to close connection: input overflow");
            return (-1);
        }
...
static int perform_subst(struct descriptor_data *t, char *orig, char *subst)
{
    ...
    char *first(NULL), *second(NULL), *strpos(NULL);
...
#ifndef MEMORY_DEBUG
    if (!tics_passed) {
        WriteLogf("SYSERR: CHECKPOINT shutdown: tics not updated. (Infinite loop suspected)");
        abort();
    }
    else {
        tics_passed = 0;
...
static RETSIGTYPE hupsig(int sig)
{
    WriteLogf("SYSERR: Received SIGHUP, SIGINT, or SIGTERM.  Shutting down...");
...
void send_to_range(room_vnum start, room_vnum finish, const char *messg, ...)
{
    ...
    if (start > finish) {
        WriteLogf("send_to_range passed start room value greater then finish.");
        return;
    }
    if (messg == NULL) {
        return;
    ...
    for (;;) {
        if (*orig == '$') {
            ...
            case '$':
                ...
                break;
            default:
                WriteLogf("SYSERR: Illegal $-code to act(): %c", *orig);
                WriteLogf("SYSERR: %s", orig);
                i = "";
                break;
            }
            while ((*buf = *(i++))) {
    ...
    struct obj_data *obj, void *vict_obj, int type)
{
    ...
    if (ch && IN_ROOM(ch) != NOWHERE) {
        ...
        WriteLogf("SYSERR: no valid target to act()!");
        return NULL;
    }
