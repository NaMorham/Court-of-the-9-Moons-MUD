 ...
 */
const int dice(const int num, const int size)
{
    int sum = 0;
 ...
 * @param minv the maximum value of the range.
 * @retval int The value limited to the range (minv <= val <= maxv).
 */
inline const int LIMIT(const int val, const int minv, const int maxv)
{
    if (maxv < minv) {
        return LIMIT(val, maxv, minv);
    }
    else {
...
char *CAP(char *txt)
{
    ...
    while ((*p == '\t' && *(p + 1)) || (*p == '\x1B' && *(p + 1) == '[')) {
        if (*p == '\t') {                  // Skip @ sign and color letter/number
            ...
            while (*p && !isalpha(*p)) {  // Skip until a 'letter' is found
                p++;
            }  // while (*p &&
            if (*p) {                     // Skip the letter
                p++;
            }
        }
    }  // while ((*p ==
...
int str_cmp(const char *arg1, const char *arg2)
{
    ...
    if ((arg1 == NULL) || (arg2 == NULL)) {
        WriteLogf("SYSERR: str_cmp() passed a NULL pointer, %p or %p.", arg1, arg2);
        return (0);
    }
...
int touch(const char *path)
{
    ...
    if (!(fl = fopen(path, "a"))) {
        WriteLogf("SYSERR: %s: %s", path, strerror(errno));
        return (-1);
    }
    else {
        fclose(fl);
...
int get_filename(char *filename, size_t fbufsize, int mode, const char *orig_name)
{
    ...
    if ((orig_name == NULL) || (*orig_name == '\0') || (filename == NULL)) {
        WriteLogf("SYSERR: NULL pointer or empty string passed to get_filename(), %p or %p.", orig_name, filename);
        return (0);
    }
...
{
    WriteLogf("SYSERR: Assertion failed at %s:%d!", who, line);
...
int room_is_dark(room_rnum room)
{
    if (!VALID_ROOM_RNUM(room)) {
        WriteLogf("room_is_dark: Invalid room rnum %d. (0-%d)", room, top_of_world);
        return (FALSE);
    }
...
void char_from_furniture(struct char_data *ch)
{
    ...
    if (!(furniture = SITTING(ch))){
        WriteLogf("SYSERR: No furniture for char in char_from_furniture.");
        SITTING(ch) = NULL;
    ...
    if (!(tempch = OBJ_SAT_IN_BY(furniture))){
        WriteLogf("SYSERR: Char from furniture, but no furniture!");
        SITTING(ch) = NULL;
        NEXT_SITTING(ch) = NULL;
