...
static void init_boards(void)
{
    ...
    for (i = 0; i < NUM_OF_BOARDS; i++) {
        if ((BOARD_RNUM(i) = real_object(BOARD_VNUM(i))) == NOTHING) {
            WriteLogf("SYSERR: Fatal board error: board vnum %d does not exist!",
                BOARD_VNUM(i));
            fatal_error = 1;
        }
        num_of_msgs[i] = 0;
...
SPECIAL(gen_board)
{
    ...
    if ((board_type = find_board(ch)) == -1) {
        WriteLogf("SYSERR:  degenerate board!  (what the hell...)");
        return (0);
    }
    if (cmd == ACMD_WRITE) {
...
int board_write_message(int board_type, struct char_data *ch, char *arg, struct obj_data *board)
{
    ...
    if ((NEW_MSG_INDEX(board_type).slot_num = find_slot()) == -1) {
        ...
        WriteLogf("SYSERR: Board: failed to find empty slot on write.");
        return (1);
    }
    // skip blanks
    skip_spaces(&arg);
...
fubar:
    WriteLogf("SYSERR: Board %d is fubar'd.", board_type);
...
void board_load_board(int board_type)
{
    ...
    for (i = 0; i < num_of_msgs[board_type]; i++)
    {
        ...
        if ((MSG_SLOTNUM(board_type, i) = find_slot()) == -1) {
            WriteLogf("SYSERR: Out of slots booting board %d!  Resetting...", board_type);
            board_reset_board(board_type);
            return;
        }
