...
void build_player_index(void)
{
    ...
    if (!(plr_index = fopen(index_name, "r"))) {
        top_of_p_table = -1;
        WriteLogf("No player index file!  First new char will be IMP!");
        return;
    }
...
void save_player_index(void)
{
    ...
    if (!(index_file = fopen(index_name, "w"))) {
        WriteLogf("SYSERR: Could not write player index file");
        return;
    }
...
int load_char(const char *name, struct char_data *ch)
{
    ...
    if ((id = get_ptable_by_name(name)) < 0) {
        ...
        GET_AC(ch) = PFDEF_AC;
        ch->real_abils.setStr(PFDEF_STR);
        ch->real_abils.setStrAdd(PFDEF_STRADD);
        ch->real_abils.setDex(PFDEF_DEX);
        ch->real_abils.setIntel(PFDEF_INT);
        ch->real_abils.setWis(PFDEF_WIS);
        ch->real_abils.setCon(PFDEF_CON);
        ch->real_abils.setCha(PFDEF_CHA);
        ...
        while (get_line(fl, line)) {
            ...
            case 'A':
                if (!strcmp(tag, "Ac  ")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'A' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'B':
                if (!strcmp(tag, "Badp")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'B' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'D':
                if (!strcmp(tag, "Desc")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'D' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'E':
                if (!strcmp(tag, "Exp ")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'E' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'F':
                if (!strcmp(tag, "Frez")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'F' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'G':
                if (!strcmp(tag, "Gold")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'G' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'H':
                if (!strcmp(tag, "Hit ")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'H' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'I':
                if (!strcmp(tag, "Id  ")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'I' tag [%s] in pfile [%s]", tag, name);
                }
                break;

            case 'J':
                mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'J' tag [%s] in pfile [%s]", tag, name);
                break;

            case 'K':
                mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'K' tag [%s] in pfile [%s]", tag, name);
                break;
            ...
            case 'L':
                if (!strcmp(tag, "Last")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'L' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'M':
                if (!strcmp(tag, "Mana")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'M' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'Q':
                if (!strcmp(tag, "Qstp")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'Q' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'S':
                if (!strcmp(tag, "Sex ")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'S' tag [%s] in pfile [%s]", tag, name);
                }
                break;
            ...
            case 'T':
                if (!strcmp(tag, "Thir")) {
                    ...
                    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'T' tag [%s] in pfile [%s]", tag, name);
                }
                break;

            case 'U':
                mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'U' tag [%s] in pfile [%s]", tag, name);
            ...
            case 'X':
                mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'X' tag [%s] in pfile [%s]", tag, name);
                break;

            case 'Y':
                mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'Y' tag [%s] in pfile [%s]", tag, name);
                break;

            case 'Z':
                mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Unknown 'Z' tag [%s] in pfile [%s]", tag, name);
                break;
...
void save_char(struct char_data * ch)
{
    ...
    if ((i >= MAX_AFFECT) && aff && aff->next) {
        WriteLogf("SYSERR: WARNING: OUT OF STORE ROOM FOR AFFECTED TYPES!!!");
    }
...
void clean_pfiles(void)
{
    ...
    for (i = 0; i <= top_of_p_table; i++) {
        ...
        if (!IS_SET(player_table[i].flags, PINDEX_NODELETE) &&
            *player_table[i].name) {
            // If the player is already flagged for deletion, then go ahead and get rid of him.
