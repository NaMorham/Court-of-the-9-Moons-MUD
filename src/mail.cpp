...
static struct mail_t *read_mail_record(FILE *mail_file)
{
    ...
    if (sscanf(line, "### %ld %ld %ld", &recipient, &sender, (long *)&sent_time) != 3) {
        WriteLogf("Mail system - fatal error - malformed mail header");
        WriteLogf("Line was: %s", line);
        return NULL;
    }
...
int scan_file(void)
{
    ...
    if (!(mail_file = fopen(MAIL_FILE, "r"))) {
        WriteLogf("   Mail file non-existant... creating new file.");
        touch(MAIL_FILE);
        return TRUE;
    }
    ...
    fclose(mail_file);
    WriteLogf("   Mail file read -- %d messages.", count);
    return TRUE;
}
