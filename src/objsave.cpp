...
#define TEST_OBJN(field) (obj->obj_flags.field != temp->obj_flags.field)
    ...
    if (obj->ex_description || temp->ex_description) {
        // To be reimplemented.  Need to handle this case in loading as well
        if ((obj->ex_description && temp->ex_description &&
...
int Crash_delete_file(char *name)
{
    ...
    if (!(fl = fopen(filename, "r"))) {
        if (errno != ENOENT) {  // if it fails but NOT because of no file
            WriteLogf("SYSERR: deleting crash file %s (1): %s", filename, strerror(errno));
        }
        return FALSE;
    }
    fclose(fl);
    ...
    if (remove(filename) < 0 && errno != ENOENT) {
        WriteLogf("SYSERR: deleting crash file %s (2): %s", filename, strerror(errno));
    }
...
int Crash_delete_crashfile(struct char_data *ch)
{
    ...
    if (!(fl = fopen(filename, "r"))) {
        if (errno != ENOENT) {  // if it fails, NOT because of no file
            WriteLogf("SYSERR: checking for crash file %s (3): %s", filename, strerror(errno));
        }
        return FALSE;
    }
...
int Crash_clean_file(char *name)
{
    ...
    if (!(fl = fopen(filename, "r"))) {
        if (errno != ENOENT) {  // if it fails, NOT because of no file
            WriteLogf("SYSERR: OPENING OBJECT FILE %s (4): %s", filename, strerror(errno));
        }
        return FALSE;
    }
    ...
    if ((rentcode == RENT_CRASH) || (rentcode == RENT_FORCED) || (rentcode == RENT_TIMEDOUT)) {
        if (timed < time(0) - (CONFIG_CRASH_TIMEOUT * SECS_PER_REAL_DAY)) {
            ...
            WriteLogf("    Deleting %s's %s file.", name, filetype);
            return TRUE;
        }
        ...
        if (timed < time(0) - (CONFIG_RENT_TIMEOUT * SECS_PER_REAL_DAY)) {
            Crash_delete_file(name);
            WriteLogf("    Deleting %s's rent file.", name);
            return TRUE;
        }
    }
    return FALSE;
...
static int Crash_is_unrentable(struct obj_data *obj)
{
    ...
    if (OBJ_FLAGGED(obj, ITEM_NORENT) ||
        ...
        WriteLogf("Crash_is_unrentable: removing object %s", obj->short_description);
        return TRUE;
    }
...
static int objsave_write_rentcode(FILE *fl, int rentcode, int cost_per_day, struct char_data *ch)
{
    if (fprintf(fl, "%d %ld %d %d %d %d\r\n", rentcode, (long)time(0),
        cost_per_day, GET_GOLD(ch), GET_BANK_GOLD(ch), 0) < 1)
    {
    ...
    while (TRUE) {
        ...
        if (get_line(fl, line) == FALSE || (*line == '$' && line[1] == '~')) {
            if (temp == NULL && current->obj == NULL)    {
                ...
                if (temp != current->obj) {
                    WriteLogf("inconsistent object pointers in objsave_parse_objects: %p/%p", temp, current->obj);
                }
            }
            break;
        }  // if (get_line(fl, line) ...
        ...
        if (*line == '#') {
            ...
            if (sscanf(line, "#%d", &nr) == 1) {
                ...
                // Coins, Corpse, etc...)
                if ((real_object(nr) == NOTHING) && (nr != NOTHING)) {   //object does not exist
                    WriteLogf("SYSERR: Prevented loading of non-existant item #%d.", nr);
                    continue;
                }
                if (temp) {
                    current->obj = temp;
            ...
            if (nr == NOTHING) {   // then it is unique
                ...
                if (real_object(nr) != NOTHING) {
                    ...
                    WriteLogf("Nonexistent object %d found in rent file.", nr);
                }
            }
            // go read next line - nothing more to see here.
            continue;
        ...
        case 'V':
            ...
            break;
        default:
            WriteLogf("Unknown tag in rentfile: %s", tag);
        }
    }
