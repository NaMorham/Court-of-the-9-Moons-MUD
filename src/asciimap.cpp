...
static struct map_info_type map_info[] =
{
    ...
    { SECT_TRACK,        "\tc[\tWr\tc]\tn" },   // show as a road for the purpose of the map (for now)
    { -1,                ""        },
    { -1,                ""        },
    { -1,                ""        },
    { -1,                ""        },           // 15
...
static struct map_info_type world_map_info[] =
{
    ...
    { SECT_UNDERWATER,   "\tbU" },
    { SECT_ROAD,         "\tWr" },  // 10
    { SECT_TRACK,        "\twr" },  // same symbol as SECT_ROAD for now
    { -1,                "" },
    { -1,                "" },
    { -1,                "" },
    { -1,                "" },      // 15
...
static void MapArea(room_rnum room, struct char_data *ch, int x, int y, int min, int max, sh_int xpos, sh_int ypos, bool worldmap)
{
    ...
    int x_exit_pos = 0;
    int y_exit_pos = 0;
    sh_int prospect_xpos;
    sh_int prospect_ypos;
    ...
    for (door = 0; door < MAX_MAP_DIR; door++) {
        ...
        if ((pexit = world[room].dir_option[door]) != NULL &&
            ...
            (!IS_SET(pexit->exit_info, EX_HIDDEN) || PRF_FLAGGED(ch, PRF_HOLYLIGHT))) {   // We have a real exit
