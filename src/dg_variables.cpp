...
void add_var(struct trig_var_data **var_list, const char *name, const char *value, long id)
{
  ...
  if (strchr(name, '.')) {
    WriteLogf("add_var() : Attempt to add illegal var: %s", name);
    return;
  }
    ...
    if (!*field) {
        if (vd) {
            ...
            if (!str_cmp(var, "self")) {
                ...
                script_log("(!vd) Unknown var [%s] on trigger [%d].", var, trig_vnum);
                *str = '\0';
            }
        }  // end   // !vd
        ...
        if (vd) {
            ...
            case MOB_TRIGGER:
                ...
                if ((o = get_object_in_equip(ch, name))) {}  // is the "name" object equipped by the mob
                else if ((o = get_obj_in_list(name, ch->carrying))) {}  // is the "name" obj carried by the mob
                else if (IN_ROOM(ch) != NOWHERE && (c = get_char_in_room(&world[IN_ROOM(ch)], name))) {}  // is the "name" char in the mob's room
                else if ((o = get_obj_in_list(name, world[IN_ROOM(ch)].contents))) {}  // is the "name" object in the mob's room
                else if ((c = get_char(name))) {}  // are we the "name" mob
                else if ((o = get_obj(name))) {}  // are we the "name" obj
                else if ((r = get_room(name))) {}  // are we the "name" room
                break;

            case OBJ_TRIGGER:
                obj = (obj_data *)go;

                if ((c = get_char_by_obj(obj, name))) {}  // get the "name" char carrying the object?
                else if ((o = get_obj_by_obj(obj, name))) {}  // get the "name" obj in? the object
                else if ((r = get_room(name))) {}  // get the "name" room with the object
                break;

            case WLD_TRIGGER:
                room = (struct room_data *) go;

                if ((c = get_char_by_room(room, name))) {}  // is the "name" char in the room
                else if ((o = get_obj_by_room(room, name))) {}  // is the "name" obj in the room
                else if ((r = get_room(name))) {}  // get the "name" room
                break;
            ...
            if (!str_cmp(var, "self")) {
                ...
                case MOB_TRIGGER:
                    c = (char_data *)go;
                    r = NULL;
                    o = NULL;   // NULL assignments added to avoid self to always be the room.  - Welcor
                    break;
                case OBJ_TRIGGER:
                    o = (obj_data *)go;
                    c = NULL;
                ...
                *str = '\0';
                ...
                if (!str_cmp(field, "hour")) {
                    ...
                    script_log("Unknown field [%s] on time var in trigger [%d].", field, trig_vnum);
                    *str = '\0';
                }
                return;
            }  // else if (!str_cmp(var, "time"))
            ...
            // Addition inspired by Jamie Nelson.
                ...
                if (!str_cmp(field, "char")) {
                    ...
                    if (in_room == NOWHERE) {
                        *str = '\0';
                        ...
                        if (!doors) {
                            *str = '\0';  // did not find a valid door/exit
                        }
                        else {
                            for (;;) {
                ...
                script_log("Unknown var [%s] on trigger [%d].", var, trig_vnum);
            }
        }  // else  //  if (vd)
    ...
    int type, char *var, char *field, char *subfield, char *str, size_t slen)
{
    ...
    *str = '\x1';
        ...
        if (!str_cmp(field, "canbeseen")) {
            ...
            if (subfield && *subfield) {
                ...
                int max = (IS_NPC(c) || GET_LEVEL(c) >= LVL_GRGOD) ? 25 : 18;
                        c->aff_abils.setCha(GET_CHA(c) + addition);
                if (GET_CHA(c) > max) {
                            c->aff_abils.setCha(max);
                }
                if (GET_CHA(c) < 3) {
                            c->aff_abils.setCha(3);
                }
            }
            ...
            if (subfield && *subfield) {
                int addition = atoi(subfield);
                int max = (IS_NPC(c) || GET_LEVEL(c) >= LVL_GRGOD) ? 25 : 18;
                        c->aff_abils.setCon(GET_CON(c) + addition);
                if (GET_CON(c) > max) {
                            c->aff_abils.setCon(max);
                }
                if (GET_CON(c) < 3) {
                            c->aff_abils.setCon(3);
                }
            }
    ...
    case 'd':
        if (!str_cmp(field, "dex")) {
            if (subfield && *subfield) {
                int addition = atoi(subfield);
                int max = (IS_NPC(c) || GET_LEVEL(c) >= LVL_GRGOD) ? 25 : 18;
                        c->aff_abils.setDex(GET_DEX(c) + addition);
                if (GET_DEX(c) > max) {
                            c->aff_abils.setDex(max);
                }
                if (GET_DEX(c) < 3) {
                            c->aff_abils.setDex(3);
                }
            }
    ...
    case 'i':
        ...
        // new check for pc/npc status
            ...
            if (subfield && *subfield) {
                int addition = atoi(subfield);
                int max = (IS_NPC(c) || GET_LEVEL(c) >= LVL_GRGOD) ? 25 : 18;
                c->aff_abils.setIntel(GET_INT(c) + addition);
                if (GET_INT(c) > max) {
                    c->aff_abils.setIntel(max);
                }
                else if (GET_INT(c) < 3) {
                    c->aff_abils.setIntel(3);
                }
            }
        ...
        if (!str_cmp(field, "saving_breath")) {
            ...
            *str = '\0'; /* so the parser know we recognize 'skillset' as a field */
            ...
            if (subfield && *subfield) {
                int addition = atoi(subfield);
                int max = (IS_NPC(c) || GET_LEVEL(c) >= LVL_GRGOD) ? 25 : 18;
                c->aff_abils.setStr(GET_STR(c) + addition);
                if (GET_STR(c) > max) {
                    c->aff_abils.setStr(max);
                }
                else if (GET_STR(c) < 3) {
                    c->aff_abils.setStr(3);
                }
            }
            ...
            if (GET_STR(c) >= 18) {
                if (subfield && *subfield) {
                    int addition = atoi(subfield);
                    c->aff_abils.setStrAdd(GET_ADD(c) + addition);
                    if (GET_ADD(c) > 100) {
                        c->aff_abils.setStrAdd(100);
                    }
                    if (GET_ADD(c) < 0) {
                        c->aff_abils.setStrAdd(0);
                    }
                }
    ...
    case 'w':
        if (!str_cmp(field, "weight")) {
            ...
            if (subfield && *subfield) {
                int addition = atoi(subfield);
                int max = (IS_NPC(c) || GET_LEVEL(c) >= LVL_GRGOD) ? 25 : 18;
                c->aff_abils.setWis(GET_WIS(c) + addition);
                if (GET_WIS(c) > max) {
                    c->aff_abils.setWis(max);
                }
                if (GET_WIS(c) < 3) {
                    c->aff_abils.setWis(3);
                }
            }
    ...
    default:
        script_log("Reached default case with field [%s] on trigger [%d].", field, trig_vnum);
    }  // switch *field
    ...
    int type, char *var, char *field, char *subfield, char *str, size_t slen)
{
    ...
    *str = '\x1';
    ...
    default:
        script_log("Unhandled field [%s] on trigger [%d].", field, trig_vnum);
    }  // switch *field
    ...
    int type, char *var, char *field, char *subfield, char *str, size_t slen)
{
    ...
    if (r->number == 0) {
        ...
        if (northDir) {
            if (subfield && *subfield) {
                if (!str_cmp(subfield, "vnum")) {
                    snprintf(str, slen, "%d", GET_ROOM_VNUM(northDir->to_room));
                }
                else if (!str_cmp(subfield, "key")) {
                    snprintf(str, slen, "%d", northDir->key);
                }
                else if (!str_cmp(subfield, "bits")) {
                    sprintbit(northDir->exit_info, exit_bits, str, slen);
                }
                else if (!str_cmp(subfield, "room")) {
                    if (northDir->to_room != NOWHERE) {
                        snprintf(str, slen, "%c%ld", UID_CHAR, (long)world[northDir->to_room].number + ROOM_ID_BASE);
                    }
                    else {
                        *str = '\0';
                    }
        ...
        room_direction_data *eastDir = R_EXIT(r, EAST);
        if (eastDir) {
            if (subfield && *subfield) {
                if (!str_cmp(subfield, "vnum")) {
                    snprintf(str, slen, "%d", GET_ROOM_VNUM(eastDir->to_room));
                }
                else if (!str_cmp(subfield, "key")) {
                    snprintf(str, slen, "%d", eastDir->key);
                }
                else if (!str_cmp(subfield, "bits")) {
                    sprintbit(eastDir->exit_info, exit_bits, str, slen);
                }
                else if (!str_cmp(subfield, "room")) {
                    if (eastDir->to_room != NOWHERE) {
                        snprintf(str, slen, "%c%ld", UID_CHAR, (long)world[eastDir->to_room].number + ROOM_ID_BASE);
                    }
                    else {
                        *str = '\0';
                    }
        ...
        room_direction_data *southDir = R_EXIT(r, SOUTH);
        if (southDir) {
            if (subfield && *subfield) {
                if (!str_cmp(subfield, "vnum")) {
                    snprintf(str, slen, "%d", GET_ROOM_VNUM(southDir->to_room));
                }
                else if (!str_cmp(subfield, "key")) {
                    snprintf(str, slen, "%d", southDir->key);
                }
                else if (!str_cmp(subfield, "bits")) {
                    sprintbit(southDir->exit_info, exit_bits, str, slen);
                }
                else if (!str_cmp(subfield, "room")) {
                    if (southDir->to_room != NOWHERE) {
                        snprintf(str, slen, "%c%ld", UID_CHAR, (long)world[southDir->to_room].number + ROOM_ID_BASE);
                    }
                    else {
                        *str = '\0';
                    }
        ...
        room_direction_data *westDir = R_EXIT(r, WEST);
        if (westDir) {
            if (subfield && *subfield) {
                if (!str_cmp(subfield, "vnum")) {
                    snprintf(str, slen, "%d", GET_ROOM_VNUM(westDir->to_room));
                }
                else if (!str_cmp(subfield, "key")) {
                    snprintf(str, slen, "%d", westDir->key);
                }
                else if (!str_cmp(subfield, "bits")) {
                    sprintbit(westDir->exit_info, exit_bits, str, slen);
                }
                else if (!str_cmp(subfield, "room")) {
                    if (westDir->to_room != NOWHERE) {
                        snprintf(str, slen, "%c%ld", UID_CHAR, (long)world[westDir->to_room].number + ROOM_ID_BASE);
                    }
                    else {
                        *str = '\0';
                    }
        ...
        room_direction_data *upDir = R_EXIT(r, UP);
        if (upDir) {
            if (subfield && *subfield) {
                if (!str_cmp(subfield, "vnum")) {
                    snprintf(str, slen, "%d", GET_ROOM_VNUM(upDir->to_room));
                }
                else if (!str_cmp(subfield, "key")) {
                    snprintf(str, slen, "%d", upDir->key);
                }
                else if (!str_cmp(subfield, "bits")) {
                    sprintbit(upDir->exit_info, exit_bits, str, slen);
                }
                else if (!str_cmp(subfield, "room")) {
                    if (upDir->to_room != NOWHERE) {
                        snprintf(str, slen, "%c%ld", UID_CHAR, (long)world[upDir->to_room].number + ROOM_ID_BASE);
                    }
                    else {
                        *str = '\0';
                    }
        ...
        room_direction_data *downDir = R_EXIT(r, DOWN);
        if (downDir) {
            if (subfield && *subfield) {
                if (!str_cmp(subfield, "vnum")) {
                    snprintf(str, slen, "%d", GET_ROOM_VNUM(downDir->to_room));
                }
                else if (!str_cmp(subfield, "key")) {
                    snprintf(str, slen, "%d", downDir->key);
                }
                else if (!str_cmp(subfield, "bits")) {
                    sprintbit(downDir->exit_info, exit_bits, str, slen);
                }
                else if (!str_cmp(subfield, "room")) {
                    if (downDir->to_room != NOWHERE) {
                        snprintf(str, slen, "%c%ld", UID_CHAR, (long)world[downDir->to_room].number + ROOM_ID_BASE);
                    }
                    else {
                        *str = '\0';
                    }
...
}  // void find_replacement_room(struct room_data *r, void *go, struct script_data *sc, ...
